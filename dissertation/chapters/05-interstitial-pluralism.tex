...
\chapter{Website Cartography: Mapping Interstitial Pluralism}
\labch{interstitial}
\margintoc

% Part reading guide margin note commented out (overlaps with margintoc)
%\footnote{\textbf{Part~II} --- \textit{Data-Driven, Theory-Guided Workflows.}\\ Five IS bridging demonstrations \parencite{maass2018data}. Each modality is a distinct material infrastructure with its own affordances and constraints --- not a neutral window onto the same underlying reality but a different sociomaterial co-constitution of which aspects of role-taking become visible \parencite{orlikowski2008sociomateriality}. Presented sequentially; built iteratively.}

\begin{chapteroverview}
This chapter operationalizes the first modality of multimodal cartography through a \textbf{website cartography workflow}: crawling and analysing 675 EIT ecosystem websites to map how innovation intermediaries textually position themselves. The Maass bridging framework runs as Curation (website crawl) $\rightarrow$ Annotation (Knowledge Triangle scoring) $\rightarrow$ Unsupervised (clustering) $\rightarrow$ Output (sociotype map)---transforming organizational self-presentation into structured evidence of role constellations.

Two theoretical contributions emerge. First, EIT Communities navigate \emph{interstitial space}: simultaneously claiming adjacency to multiple institutional logics (education, research, business) without fully belonging to any. Second, five distinctive \emph{sociotypes} characterize how intermediaries enact this interstitiality---stable relational configurations that persist across organisations and domains. A \emph{Knowledge Triangle imbalance} (business 49\%, research 37\%, education 14\%) challenges the EIT's formal integration mandate.
\end{chapteroverview}
...
A sociomaterial caveat is warranted at the outset.\sidenote{\textbf{Platform materiality}---Websites are not transparent windows onto organisational intent but material-semiotic assemblages whose architectures shape what kinds of role claims become articulable. See Section~\ref{sec:background:materiality}.} Websites are not transparent windows onto organisational intent. They are material-semiotic assemblages where content management architectures, navigation hierarchies, and visual templates co-produce the role profiles we analyse. The Knowledge Triangle's neat tripartition into education, research, and business is partly an artifact of how websites organise information: discrete menu items and landing pages encourage categorical self-presentation that may over-emphasise distinctions relative to the messier hybridity of actual practice. We treat this not as a limitation but as a feature of the sociomaterial world worth attending to---the \emph{medium} shapes the \emph{message} about roles.

\begin{replicationbox}[Computational Artifacts]
\faLaptopCode\ \textbf{Interactive UIs} (\texttt{rolebox-websites/ui/}):
\begin{itemize}[leftmargin=*, itemsep=0.1em]
    \item \texttt{rolebox-triads.html} --- Knowledge Triangle positioning with X-Curve dynamics
    \item \texttt{rolebox-sociotypes.html} --- Role configuration networks and community clusters
    \item \texttt{network-layouts.html} --- Hyperlink network visualization (6 layout algorithms)
\end{itemize}
\faGithub\ \textbf{Code \& Data:} \texttt{github.com/babajideowoyele/rolebox-websites}\\
\faBook\ \textbf{Documentation:} \texttt{README.md} and \texttt{ui/README.md} in repository
\end{replicationbox}
...
After preprocessing and URL deduplication,\sidenote{\textbf{Deduplication}---removing duplicate crawls of identical pages and standardizing URL variants ensures each unique webpage is analyzed once.} the corpus comprised 2,190 crawled websites (1,700 with extractable text) totaling approximately 583,000 words.
...
\begin{tikzpicture}[scale=1.2]
    ...
    % Percentage labels at vertices (computed from field_positions_full.csv, n=1,298)
    \node[font=\small, text=DarkBlue] at (0.6,0.5) {14\%};
    \node[font=\small, text=DarkBlue] at (5.4,0.5) {49\%};
    \node[font=\small, text=DarkBlue] at (3,4.2) {37\%};

    % Center point (actual position: x = 0.49*6 + 0.37*3 = 4.05, y = 0.37*5.2 = 1.92)
    \fill[AccentRed] (4.05,1.92) circle (4pt);
    \node[font=\scriptsize, right] at (4.15,1.92) {EIT aggregate};
...
Despite mandated knowledge triangle ``integration,'' EIT website discourse tilts strongly toward business/entrepreneurship (49\%), with research (37\%) and education (14\%) receiving less emphasis. Education is particularly underrepresented relative to its nominal role in the knowledge triangle mandate.
\end{insightbox}
...
\toprule
\textbf{KIC} & \textbf{$n$} & \textbf{Education} & \textbf{Research} & \textbf{Business} & \textbf{Profile} \\
\midrule
Climate-KIC & 6 & 1\% & 21\% & 77\% & Business-tilted \\
...
EIT Urban Mobility & 111 & 22\% & 35\% & 43\% & Balanced-business \\
\midrule
\textbf{Total} & \textbf{1,298} & \textbf{14\%} & \textbf{37\%} & \textbf{49\%} & \\
\bottomrule
\end{tabular}
\captionof{table}{Knowledge triangle positioning by KIC (computed from website keyword analysis, $n=1{,}298$ organizations)}
\labtab{kt-by-kic}
\end{center}
...
\begin{itemize}
    \item \textbf{Strongest business tilt}: Climate-KIC (77\%) and InnoEnergy (63\%) show the most pronounced entrepreneurial orientation, though Climate-KIC's small sample ($n=6$) warrants caution

    \item \textbf{Research-tilted}: EIT Health stands out with 49\% research emphasis---the only KIC where research exceeds business---reflecting the sector's evidence-based culture. EIT Food (38\%) also shows strong research orientation

    \item \textbf{Most balanced}: EIT Urban Mobility (22/35/43\%) shows the most even distribution, perhaps reflecting the multi-stakeholder nature of urban planning

    \item \textbf{Education underrepresented}: Education scores are low across all KICs (1--22\%), with InnoEnergy (5\%) and Climate-KIC (1\%) particularly striking given their Master's and doctoral programmes
\end{itemize}
...
The entrepreneurial tilt raises questions about the EIT model's realization of knowledge triangle integration:\footnote{If integration means absorption into business discourse, then the knowledge triangle becomes a rhetorical frame rather than a structural reality.}
...
Role identities are inherently relational: they are constituted through typified actions directed toward typified others.\footnote{A ``teacher'' implies ``students'' and practices of instruction; a ``service provider'' implies ``customers'' and practices of delivery; an ``investor'' implies ``investees'' and practices of funding.} Following Berger and Luckmann \cite{berger1967social}, we understand this reciprocal typification of actor and act as the core of institutionalization.
...
\textbf{Interconnectedness} refers to the relationships between configurations within a constellation.\footnote{Configurations may be densely or sparsely connected; connections may be evenly distributed or concentrated at particular interfaces.} We distinguish:
...
Extracted triplets undergo normalization to enable aggregation:\footnote{Normalization maps surface variation to canonical categories while preserving semantic distinctions.}
...
To assess extraction precision, a stratified sample of 81 triplets
(drawn proportionally across all eight KICs) was independently evaluated
for semantic validity.\footnote{Validation assessed whether the extracted
\emph{role identity--practice--counterrole} relation faithfully represents
the source sentence.}
Of the 81 triplets, 71 (87.7\%) were judged fully valid,
10 (12.3\%) were rejected as parsing artefacts or over-generalizations,
and none required partial modification---indicating that
the dependency-based extraction pipeline produces high-precision triplets
at the cost of conservative recall.
...
Analysis of the aggregate network reveals a modular constellation comprising three primary sociotype configurations.\sidenote{\textbf{Three configurations}---the number emerged from community detection on 3,774 extracted role triplets using Louvain clustering.} The constellation exhibits low differentiation ($D = 0.00$) and moderate-to-high interconnectedness ($I = 0.56$), indicating that EIT Communities instantiate plural relational structure with substantial overlap between configurations.

\subsection{Sociotype Profiles}

We present profiles of the three configurations, proceeding from highest to lowest permeability.
...
\end{tikzpicture}
\captionof{figure}{The three sociotypes of EIT innovation intermediation. Permeability scores ($P$) indicate boundary porosity; dashed lines indicate interface density between configurations.}
\labfig{three-sociotypes}
\end{center}
\end{wideblock}
...
The positioning analysis revealed that EIT Communities occupy interstitial positions in the knowledge triangle, though tilted toward business. The sociotype analysis reveals \textit{how} they navigate this interstitiality: through a constellation of three sociotypes with varying permeability and interconnection.

The connection between positioning and sociotypes is not coincidental:\footnote{The entrepreneurial tilt in discourse corresponds to the centrality of the Acceleration sociotype.}
...
The \textbf{positioning analysis} mapped the discursive positioning of EIT Communities within the knowledge triangle, revealing systematic \textbf{business tilt}: business discourse leads (49\%) while research (37\%) and education (14\%) receive less emphasis. Cross-KIC variation is substantial, ranging from business-dominant (Climate-KIC at 77\%, InnoEnergy at 63\%) to research-tilted (EIT Health at 49\% research) to balanced-business (Urban Mobility at 22/35/43\%).
...
\labsec{interstitial:designknowledge}

Beyond its empirical results, this chapter contributes three transferable design principles for multimodal cartography workflows that map discursive positioning and relational role configurations from organizational self-presentations:

\begin{description}
    \item[DP-W1: Vocabulary-guided field mapping.] Applying a theory-derived lexicon (343 KT terms spanning education, research, and business registers) to organizational self-presentations enables scalable discursive positioning without requiring manual annotation of full corpora. The operationalization demonstrates MDR-2 (theory-guided operationalization): substantive theory directly shapes which signals are counted, ensuring that measurement remains interpretively grounded.

    \item[DP-W2: Role triple extraction as relational curation.] Extracting Agent--Verb--Object triples from website text and projecting them onto shared role vocabulary objects constructs relational representations from inherently non-relational sources. This operationalizes MDR-1 (multi-source integration) at the organizational level, enabling network analysis on corpora that contain no explicit relational data.

    \item[DP-W3: Network-derived typology construction.] Deriving sociotype categories through community detection on role co-occurrence networks---rather than imposing a priori role labels---enables field-specific role configurations to emerge from the data. The resulting sociotype taxonomy is more parsimonious than deductively imposed categories and captures configurations (e.g., the Accountability sociotype's peripheral position) invisible to predefined classification schemes.
\end{description}

Together, these principles demonstrate that website corpora---typically treated as qualitative case material---can serve as the foundation for scalable, replicable cartographic workflows that reveal both discursive positioning and relational role structure simultaneously.

%═══════════════════════════════════════════════════════════════════════════════
\section{Chapter Summary}
%═══════════════════════════════════════════════════════════════════════════════

% Findings summary temporarily commented out; see Design Knowledge Contribution above
%\begin{summarybox}
...
\item \textbf{Raw data}: EIT website corpus (2,190 websites, 583K words; 3,774 role triplets)
\end{itemize}
