...
% Figure 2.1: Actor vs. role constellations - enlarged
\begin{center}
\begin{tikzpicture}[scale=1.3]
    % Actor constellation (left)
    \node[font=\normalsize\bfseries] at (-4, 4) {Actor Constellation};
    \node[draw, circle, fill=Slate!20, minimum size=1.2cm] (a1) at (-6, 1.8) {};
    \node[draw, circle, fill=Slate!20, minimum size=1.2cm] (a2) at (-4, 2.8) {};
    \node[draw, circle, fill=Slate!20, minimum size=1.2cm] (a3) at (-2, 1.8) {};
    \node[draw, circle, fill=Slate!20, minimum size=1.2cm] (a4) at (-4, 0.6) {};
    \node[font=\small\itshape, text=Slate] at (-4, -0.8) {Who is present?};

    % Arrow between
    \draw[->, very thick, AccentPurple!60, line width=2pt] (-0.5, 1.5) -- (0.5, 1.5);

    % Role constellation (right)
    \node[font=\normalsize\bfseries] at (4, 4) {Role Constellation};
    \node[draw, circle, fill=Azure!30, minimum size=1.2cm, font=\small\bfseries] (r1) at (2, 2.2) {B};
    \node[draw, circle, fill=Marigold!30, minimum size=1.2cm, font=\small\bfseries] (r2) at (4, 2.8) {F};
    \node[draw, circle, fill=PandaBamboo!30, minimum size=1.2cm, font=\small\bfseries] (r3) at (6, 2.2) {C};
    \node[draw, circle, fill=AccentPurple!30, minimum size=1.2cm, font=\small\bfseries] (r4) at (4, 0.6) {A};
    \draw[thick, PandaCharcoal!50] (r1) -- (r2) -- (r3);
    \draw[thick, PandaCharcoal!50] (r2) -- (r4);
    \draw[dashed, PandaCharcoal!30] (r1) -- (r4) -- (r3);
    \node[font=\small\itshape, text=Slate] at (4, -0.8) {What do they do?};

    % Legend
    \node[font=\tiny, text=Slate] at (2, -1.5) {B=Broker};
    \node[font=\tiny, text=Slate] at (3.5, -1.5) {F=Funder};
    \node[font=\tiny, text=Slate] at (4.8, -1.5) {C=Connector};
    \node[font=\tiny, text=Slate] at (6.2, -1.5) {A=Advocate};
\end{tikzpicture}
\captionof{figure}{Actor constellations identify \emph{who} is present; role constellations map \emph{what} they do and how functions interrelate. The same actors (left) can be analyzed through different role lenses (right), revealing functions (B=Broker, F=Funder, C=Connector, A=Advocate) and relationships between them.}
...
How do the major transition frameworks conceptualize roles? Each offers a partial map with characteristic strengths and blind spots.\sidenote{\textbf{Different lenses}---Each framework illuminates some roles while obscuring others.}

% Theoretical frameworks comparison - scaled up for better visibility
\begin{center}
\begin{tikzpicture}[scale=1.0,
    fwbox/.style={draw, rounded corners=4pt, fill=#1!25, minimum width=3.2cm, minimum height=1.4cm, align=center, font=\small}
]
    \node[fwbox=Azure] (mlp) at (0,0) {\textbf{MLP}\\[0.1em]\scriptsize Niche--Regime\\dynamics};
    \node[fwbox=PandaBamboo] (tm) at (4,0) {\textbf{TM}\\[0.1em]\scriptsize Frontrunner\\arenas};
    \node[fwbox=Marigold] (snm) at (8,0) {\textbf{SNM}\\[0.1em]\scriptsize Network\\builders};
    \node[fwbox=AccentPurple] (tis) at (12,0) {\textbf{TIS}\\[0.1em]\scriptsize System\\functions};

    % Connecting lines showing overlap
    \draw[<->, dashed, Slate!50, thick] (mlp) -- (tm);
    \draw[<->, dashed, Slate!50, thick] (tm) -- (snm);
    \draw[<->, dashed, Slate!50, thick] (snm) -- (tis);

    % Key roles below each
    \node[font=\tiny, text=Slate, text width=3cm, align=center] at (0,-1.2) {Niche actors vs.\\regime incumbents};
    \node[font=\tiny, text=Slate, text width=3cm, align=center] at (4,-1.2) {Visionaries in\\protected spaces};
    \node[font=\tiny, text=Slate, text width=3cm, align=center] at (8,-1.2) {Expectation managers,\\cosmopolitan actors};
    \node[font=\tiny, text=Slate, text width=3cm, align=center] at (12,-1.2) {Entrepreneurs,\\legitimators};
\end{tikzpicture}
\captionof{figure}{Four major transition frameworks emphasize different role categories. Each illuminates some actors while obscuring others, and boundaries between frameworks are porous rather than fixed.}
...
\begin{margintable}
\centering
\caption{Agency types and transition roles}
\label{tab:background:agency-roles}
\scriptsize
...
% Figure 2.2: Intermediary function clusters
\begin{center}
\begin{tikzpicture}[scale=1.3]
    % Central intermediary
    \node[draw, circle, fill=AccentPurple!30, minimum size=1.6cm, font=\normalsize\bfseries] (I) at (0,0) {I};

    % Function nodes - larger with more padding
    \node[draw, rounded corners=4pt, fill=Azure!20, font=\small, align=center, minimum width=2.2cm, minimum height=0.9cm] (K) at (-3, 2) {Knowledge};
    \node[draw, rounded corners=4pt, fill=PandaBamboo!20, font=\small, align=center, minimum width=2.2cm, minimum height=0.9cm] (N) at (3, 2) {Network};
    \node[draw, rounded corners=4pt, fill=Marigold!20, font=\small, align=center, minimum width=2.2cm, minimum height=0.9cm] (V) at (-3, -2) {Vision};
    \node[draw, rounded corners=4pt, fill=PandaRose!20, font=\small, align=center, minimum width=2.2cm, minimum height=0.9cm] (In) at (3, -2) {Institutional};

    % Connections - thicker arrows
    \draw[very thick, Azure!70, ->] (I) -- (K);
    \draw[very thick, PandaBamboo!70, ->] (I) -- (N);
    \draw[very thick, Marigold!70, ->] (I) -- (V);
    \draw[very thick, PandaRose!70, ->] (I) -- (In);
\end{tikzpicture}
...
% Four levels of intermediation visualization - scaled up
\begin{center}
\begin{tikzpicture}[scale=1.1,
    levelbox/.style={draw, rounded corners=4pt, fill=#1!20, minimum width=4cm, minimum height=1.2cm, align=center, font=\small}
]
    % Levels as stacked boxes
    \node[levelbox=AccentPurple] (l3) at (0,4.5) {\textbf{Level 3}\\Actors--Institutions};
    \node[levelbox=Marigold] (l2) at (0,2.8) {\textbf{Level 2}\\Between networks};
    \node[levelbox=PandaBamboo] (l1) at (0,1.1) {\textbf{Level 1}\\Within network};
    \node[levelbox=Azure] (l0) at (0,-0.6) {\textbf{Level 0}\\Bilateral};

    % Examples on right with better formatting
    \node[font=\scriptsize, text width=4.5cm, right=0.8cm of l3, align=left] {\faLandmark~Lobbying, vision articulation,\\policy translation};
    \node[font=\scriptsize, text width=4.5cm, right=0.8cm of l2, align=left] {\faProjectDiagram~Coalition building,\\interaction spaces, common voice};
    \node[font=\scriptsize, text width=4.5cm, right=0.8cm of l1, align=left] {\faUsers~Networking, knowledge transfer,\\legitimacy building};
    \node[font=\scriptsize, text width=4.5cm, right=0.8cm of l0, align=left] {\faHandshake~Consulting, bilateral brokerage,\\one-to-one matching};

    % Vertical arrow showing increasing systemic scope
    \draw[->, very thick, Slate!70, line width=2pt] (-3,-0.6) -- (-3,4.5);
    \node[font=\small\bfseries, text=Slate, rotate=90, anchor=south] at (-3.5,2) {Systemic Scope};

    % In-between what? label
    \node[font=\scriptsize\itshape, text=Slate] at (0,-1.5) {``In-between what?'' determines intermediation level};
\end{tikzpicture}
\captionof{figure}{Four levels of intermediation based on the relational criterion ``in-between what?'' (Kanda et al., 2020). Level 0 involves bilateral exchanges between individual entities. Level 1 mediates within a single network. Level 2 spans different network types. Level 3 transcends horizontal network interactions to engage with formal and informal institutions.}
\label{fig:background:kanda-levels}
\end{center}
...
% Designed vs organic data visualization - moved to body for emphasis
\begin{center}
\begin{tikzpicture}[scale=1.0,
    databox/.style={draw, rounded corners=4pt, fill=#1!25, minimum width=3.5cm, minimum height=1.4cm, align=center, font=\small}
]
    % Designed data (dominant)
    \node[databox=Azure] (des) at (-3,0) {\faFile*\\[0.2em]\textbf{Designed Data}\\[0.1em]\scriptsize 82\% of studies};
    \node[font=\scriptsize, text width=3cm, align=center, below=0.3cm of des] {Interviews, surveys,\\focus groups};

    % Organic data (underused)
    \node[databox=Marigold] (org) at (3,0) {\faGlobe\\[0.2em]\textbf{Organic Data}\\[0.1em]\scriptsize <5\% of studies};
    \node[font=\scriptsize, text width=3cm, align=center, below=0.3cm of org] {Websites, social media,\\news, investments};

    % Arrow showing the gap/opportunity
    \draw[->, very thick, AccentPurple!70, line width=2pt] (des.east) -- (org.west) node[midway, above, font=\small\bfseries, text=AccentPurple] {Methodological Gap};

    % Scale indicators
    \node[font=\tiny, text=Slate, above=0.5cm of des] {Dozens of actors};
    \node[font=\tiny, text=Slate, above=0.5cm of org] {Thousands of actors};
\end{tikzpicture}
\captionof{figure}{The dominance of designed data in transitions research. While 82\% of studies rely on interviews and surveys (reaching dozens of actors), organic data sources like websites and social media (capable of reaching thousands) remain largely untapped. This methodological gap motivates the multimodal cartographic approach.}
\label{fig:background:data-types}
\end{center}
...
More striking than the divergences are the gaps all four frameworks share.\sidenote{\textbf{Six gaps}---These limitations motivate the cartographic approach developed in this dissertation.}
...
% Figure 2.3: Six problems and cartographic responses
\begin{center}
...
%===============================================================================
\section{A Seventh Problem: Platform Materiality}[Platform Materiality]
\labsec{background:materiality}
%===============================================================================

The six problems identified above share an unstated assumption: that roles exist independently of the media through which they are observed, performed, and contested. Interviews capture role perceptions; websites display role claims; news articles attribute role identities---but the \emph{media themselves} are treated as neutral channels, transparent windows onto an underlying social reality. This assumption deserves scrutiny.

Consider the empirical landscape of contemporary role research. Organizations perform roles \emph{through} digital platforms: they claim identities on websites, build networks on Twitter/X, attract investment documented in Crunchbase, receive media attributions in online news, and construct visual identities through image-rich communications.\sidenote{\textbf{Digital platforms as role infrastructure}---Each platform is not merely a data source for researchers but a material infrastructure through which organizations \emph{enact} roles.} Each platform is not simply a recording device that captures pre-existing roles. It is a \textbf{material infrastructure} with specific affordances, constraints, and constitutive effects on what roles become visible, legible, and enactable.

A website's content management system shapes what kinds of self-presentation are possible: discrete pages encourage categorical role claims (``our research,'' ``our education programmes,'' ``our startup portfolio'') that may over-emphasise neat distinctions relative to messier practice. Twitter's network architecture---follows, lists, retweets, mentions---makes certain relational patterns observable while foreclosing others. Crunchbase's standardised database schema renders financial and sectoral dimensions legible while making relational and discursive dimensions invisible. News editorial conventions of newsworthiness and balance shape which actors appear, in which roles, and with what valence.

This is the seventh problem: \textbf{the platforms through which we observe and perform roles are themselves material participants in role constitution, not neutral conduits.}

\subsection{The Material Turn in Information Systems}

The ``material turn'' in Information Systems provides the metatheoretical vocabulary to articulate this problem \cite{Orlikowski2007,orlikowski2008sociomateriality}.\sidenote{\textbf{Constitutive entanglement}---Orlikowski (2007): ``The social and the material are constitutively entangled in everyday life.'' This is not merely a claim about technology; it is an ontological stance about how the social world is produced.} \textcite{Orlikowski2007} argues that the social and the material are \emph{constitutively entangled}---not two separate domains that occasionally interact, but dimensions of a single reality that cannot be meaningfully separated. Applied to role research, this means that organisational roles are not performed in a material vacuum and then \emph{recorded} by platforms. They are \emph{co-produced} through the entanglement of organisational strategy and platform materiality.

\textcite{leonardi2012materiality} offers a complementary analytical framework through the concept of \emph{imbrication}: human agency and material agency interlock like bricks, each constraining and enabling the other. Applied to our context, organisations exercise strategic agency in how they present roles---but this agency is imbricated with platform materiality. You cannot claim to be a ``connector'' on Crunchbase; the database schema does not afford that vocabulary. You \emph{can} claim it on a website, where editorial control is maximal. The difference is not strategic but material.

This problem is not merely methodological---it is ontological. If platforms co-constitute roles, then multimodal divergence (the connector gap, the knowledge triangle imbalance, the visual-verbal disconnect) is not a limitation to be overcome through better triangulation. It is a \emph{feature} of the sociomaterial world to be theorised. Different platforms produce different role realities because their material infrastructures afford different kinds of role enactment. The \emph{pattern of divergence} across platforms becomes itself analytically informative.

\subsection{Sustainability Transitions Meet Digital Transformation}

The entanglement of platform materiality and organisational roles connects to a larger structural condition: sustainability transitions and digital transformation are themselves constitutively entangled \cite{owoyele2025datacentering}.\sidenote{\textbf{Dual entanglement}---Digital infrastructure is simultaneously infrastructure \emph{for} transitions (smart grids, precision agriculture) and infrastructure \emph{requiring} transition (decarbonisation, circularity, justice). See \textcite{owoyele2025datacentering}.} The digital platforms through which intermediary organisations perform their roles---and through which researchers like us observe those performances---are embedded in material infrastructures (data centres, undersea cables, cloud computing stacks) that consume vast energy and resources. Data centres alone consume over 400 TWh annually, more electricity than many nations.

This creates a distinctive reflexive condition. We study sustainability transition actors \emph{through} digital infrastructure that itself \emph{requires} sustainability transition. The platforms that make organisational roles observable are not neutral instruments but part of the very socio-technical system whose transformation we seek to understand. Information Systems, as the discipline that has theorised this entanglement most systematically \cite{cecezkecmanovic2014sociomateriality,plantin2018infrastructure}, provides the metatheoretical paradigm for navigating it.

\subsection{Cartographic Response to Platform Materiality}

The cartographic metaphor introduced above is perfectly suited to this seventh problem. Just as map projections distort territory---the Mercator projection inflates polar regions while shrinking equatorial ones---each digital modality ``projects'' role constellations through its own material properties.\sidenote{\textbf{Cartographic projections}---Every map projection distorts; the question is \emph{which} distortions are acceptable for \emph{which} purposes. Multimodal cartography makes projection choices explicit.} A website-based map inflates claimed roles; a network-based map inflates structural positions; an investment-based map inflates financial dimensions. No single projection captures ``true'' roles. But comparing projections---attending to where they converge and where they diverge---reveals the material conditions under which role knowledge is produced.

This seventh awareness---that the maps we draw are shaped by the material properties of the instruments we use to draw them---runs through the empirical chapters that follow. It surfaces most explicitly in the cross-modal divergence patterns discussed in Chapter~\ref{ch:synthesis}, where we develop a full sociomaterial account of why different modalities produce systematically different role profiles.
...
\item A \textbf{seventh problem}---platform materiality---recognises that digital platforms through which roles are observed and performed are not neutral conduits but material infrastructures that co-constitute role visibility. The ``material turn'' in Information Systems provides the metatheoretical vocabulary for this awareness.

\item The field's core limitation is \textbf{categorical thinking} where \textbf{cartographic thinking} is needed.
\end{itemize}

\noindent These seven problems directly motivate our first sub-research question: \textit{What conceptual framework enables theory-guided but empirically open analysis of role constellations?} The literature provides conceptual vocabulary---role constellations, intermediary functions, ecologies---but lacks operationalization at scale. The cartographic reframings outlined above constitute this dissertation's theoretical response, grounded in an Information Systems paradigm that takes the materiality of digital platforms seriously.
\end{summarybox}
