...
\begin{tikzpicture}[scale=1.2]
    ...
    \node[font=\small\bfseries] at (-4, 3.5) {Actor Constellation};
    \node[draw, circle, fill=Slate!20, minimum size=1.0cm] (a1) at (-6, 1.5) {};
    \node[draw, circle, fill=Slate!20, minimum size=1.0cm] (a2) at (-4, 2.5) {};
    \node[draw, circle, fill=Slate!20, minimum size=1.0cm] (a3) at (-2, 1.5) {};
    \node[draw, circle, fill=Slate!20, minimum size=1.0cm] (a4) at (-4, 0.5) {};
    \node[font=\small, text=Slate] at (-4, -0.8) {Who is present?};

    % Role constellation (right)
    \node[font=\small\bfseries] at (4, 3.5) {Role Constellation};
    \node[draw, circle, fill=Azure!30, minimum size=1.0cm, font=\small] (r1) at (2, 2) {B};
    \node[draw, circle, fill=Marigold!30, minimum size=1.0cm, font=\small] (r2) at (4, 2.5) {F};
    \node[draw, circle, fill=PandaBamboo!30, minimum size=1.0cm, font=\small] (r3) at (6, 2) {C};
    \node[draw, circle, fill=AccentPurple!30, minimum size=1.0cm, font=\small] (r4) at (4, 0.5) {A};
    \draw[thick, PandaCharcoal!50] (r1) -- (r2) -- (r3);
    \draw[thick, PandaCharcoal!50] (r2) -- (r4);
    \draw[dashed, PandaCharcoal!30] (r1) -- (r4) -- (r3);
    \node[font=\small, text=Slate] at (4, -0.8) {What do they do?};
\end{tikzpicture}

\captionof{figure}{Actor vs. role constellations. The former identifies presence; the latter maps functions and relationships.}
...
\textcite{wittmayer2017roles} made the seminal contribution\sidenote{\textbf{Wittmayer et al. (2017)}---Introduces sociological role theory to transition studies.} of explicitly importing sociological role theory into sustainability transitions research. Drawing on \textcite{biddle1986role}, they distinguish multiple conceptualizations of ``role'':
...
\textcite{dehaan2018power} address a widely acknowledged gap\sidenote{\textbf{De Haan \& Rotmans (2018)}---Power-in-transition framework analyzes how power shapes role possibilities.}: the inadequate representation of actors and agency in transitions studies. They propose giving ``explanatory primacy to agency,'' presenting transitions as consequences of deliberate, value-driven actions rather than systemic forces.
...
The most influential recent contribution is \textcite{kivimaa2019intermediaries},\sidenote{\textbf{Kivimaa et al. (2019)}---Systematic review proposes five-type intermediary typology.} proposing a five-type typology:
...
Each approach illuminates some aspects\sidenote{\textbf{Methodological tensions}---Depth vs. scale; meaning vs. pattern; theory vs. data.} while obscuring others. No single method captures the full terrain of intermediary role constellations.
...
Most consequentially, all four frameworks share several limitations:\sidenote{\textbf{Six gaps}---These limitations motivate the cartographic approach developed in this dissertation.}
...
Existing typologies emphasize one dimension while backgrounding others.\sidenote{\textbf{Three dimensions}---Existing typologies emphasize one dimension while backgrounding others.} A comprehensive actor/role framework would characterize positions along all three simultaneously---specifying \textit{what} actors do, \textit{where} they are positioned relationally, and \textit{how} this changes over time.
...
The \textbf{Intermezzo} that follows introduces the empirical context for developing cartographic methods:\sidenote{\faArrowRight\ \textit{Next}: \textbf{Intermezzo}---The EIT Ecosystem. An introduction to the eight Knowledge and Innovation Communities that serve as this dissertation's empirical laboratory.} the European Institute of Innovation and Technology (EIT) and its eight Knowledge and Innovation Communities. These intermediary organizations---spanning climate, energy, food, health, digital, urban mobility, raw materials, and manufacturing---provide an ideal laboratory for multimodal cartography.
