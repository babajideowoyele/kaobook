...
\chapter{Video Cartography: From Privacy-Preserving Masking to Behavioral Discovery}
\labch{video}
\margintoc
...
The main question of this dissertation asks how workflows should be designed to enable multimodal cartography of interstitial role constellations. This chapter demonstrates that the answer extends beyond the five modalities studied in preceding chapters: the same workflow-centric DSR logic---Curation $\rightarrow$ Annotation $\rightarrow$ Unsupervised $\rightarrow$ Output---transfers to \textbf{video}, the most information-dense yet most ethically constrained modality available to social science. Three iterating computational artifacts (Masked-Piper, MaskAnyone, AMP Pipeline) form a design cycle that addresses the three barriers to video research: privacy, computational cost, and pipeline fragmentation. Each artifact produces design knowledge in the Maass bridging sense \parencite{maass2018data}: not a description of video data, but an infrastructure through which behavioral phenomena become observable, measurable, and shareable. This transferability is the chapter's central theoretical claim---workflow-centric DSR is a domain-agnostic methodology for making the invisible visible.
\end{chapteroverview}

\section{Introduction: The Seventh Modality}[Introduction]
\labsec{video:intro}

The preceding chapters have mapped role constellations across five digital modalities: organizational websites (Chapter~\ref{ch:interstitial}), social media networks (Chapter~\ref{ch:rolefield}), investment records (Chapter~\ref{ch:venture}), visual materials (Chapter~\ref{ch:visual}), and news media coverage (Chapter~\ref{ch:power}).\footnote{The five modalities were selected because they represent distinct \textit{material infrastructures} through which organizational roles become observable---each with its own affordances, constraints, and constitutive effects on role performance (see Section~\ref{sec:background:materiality}).} Each chapter has showcased a computational workflow following the Maass bridging framework: Curation $\rightarrow$ Annotation $\rightarrow$ Unsupervised Analysis $\rightarrow$ Output. Each workflow transforms platform-specific digital traces into design knowledge about organizational roles.
...
These three artifacts are not hypothetical or illustrative. They are \textbf{peer-reviewed, published, and open-source contributions} developed in interdisciplinary collaboration across the Hasso Plattner Institute for Digital Engineering (Potsdam), the Donders Centre for Cognition at Radboud University (Nijmegen), and the Max Planck Institute for Psycholinguistics (Nijmegen).\sidenote{The collaboration spans two distinct research traditions---Information Systems (HPI) and cognitive/behavioral science (Donders, MPI)---which is itself a demonstration that workflow-centric DSR is a domain-crossing methodology.} Their publication at a flagship software journal (\textit{SoftwareX}), the premier IS conference (ICIS), and a flagship psychology journal (\textit{American Psychologist}) attests to the legitimacy and impact of this line of work across disciplinary communities.

Three research questions guide the chapter:

\begin{enumerate}
    \item \textbf{RQ1}: What \textit{workflow architecture} enables privacy-preserving transformation of raw video into shareable analytical outputs, and how do design requirements for video workflows relate to the Meta-Design Requirements developed for text-based modalities?
    \item \textbf{RQ2}: How does the Maass bridging framework (Curation $\rightarrow$ Annotation $\rightarrow$ Unsupervised $\rightarrow$ Output) instantiate for audio-visual data, and what \textit{discoveries} does the resulting workflow enable that manual observation cannot?
    \item \textbf{RQ3}: What \textit{transferable design knowledge} emerges from iterating across three video workflow artifacts, and what does this transferability demonstrate about the generality of workflow-centric DSR?
\end{enumerate}

\begin{replicationbox}[Publications from this Chapter]
\textbf{Three peer-reviewed open-science contributions}, developed with colleagues at HPI Potsdam, Donders Centre (Radboud), and MPI for Psycholinguistics.

\medskip
\faLaptopCode\ \textbf{Masked-Piper} \hfill \textit{SoftwareX 20, 101236 (2022)}\\
Masking personal identities in visual recordings while preserving multimodal information\\
\textit{Owoyele, Trujillo, de Melo \& Pouw} \parencite{owoyele2022maskedpiper}\\
\faGithub\ \texttt{github.com/WimPouw/TowardsMultimodalOpenScience}

\medskip
\faLaptopCode\ \textbf{MaskAnyone Toolkit} \hfill \textit{ICIS 2024, Bangkok --- Completed Research}\\
Strategies for minimizing privacy risks and maximizing utility in audio-visual data archiving\\
\textit{Owoyele, Schilling, Sawahn, Kaemer, Zherebenkov, Verma, Pouw \& de Melo} \parencite{owoyele2024maskanyone}\\
\faGithub\ \texttt{github.com/MaskAnyone/MaskAnyone}

\medskip
\faLaptopCode\ \textbf{AMP Pipeline} \hfill \textit{American Psychologist --- Special Issue (under review)}\\
An open-source standardized pipeline for equitable observations of interactive behavioral dynamics\\
\textit{Owoyele, Pouw et al.} \parencite{owoyele2025amppipeline}\\
\faCode\ \texttt{anonymous.4open.science/w/InterPerDynPipeline-347F}
\end{replicationbox}
...
Behavioral observation has a longstanding tradition in the life and social sciences, used at least since Darwin's work on emotion \parencite{darwin1872expression}.\footnote{\textbf{Manual coding limitations}---Observational methods often rely on ``coding schemes'' that introduce biases from the inherent limits of coding categories and from the subjectivity of human raters \parencite{owoyele2025amppipeline}. Computer vision methods have promise in this regard: pose estimation approaches are becoming increasingly precise, non-intrusive, reproducible, and low-cost.} Observational methods form the core of many studies across disciplines including ethology, medicine, cultural anthropology, and multiple areas of psychology. Yet these methods remain costly and prone to the biases inherent in manual coding---human observers must be trained, inter-rater reliability must be established, and the temporal granularity of observation is limited by human perceptual capacity.
...
The design requirements were elicited through two live demonstration workshops: one at a Dutch university ($n = 16$) with researchers and one at a German university ($n = 10$) with researchers and data stewards ($n = 5$).\footnote{\textbf{Stakeholder-driven design}---The participatory requirement elicitation follows DSR best practices, ensuring that the artifact addresses actual practitioner needs rather than assumed technical challenges.} Eight requirements emerged:
...
MaskAnyone employs a Manager-Worker architecture: the backend serves as the central hub for user interactions and job management, while Docker-containerized workers handle the computational processing.\footnote{The Manager-Worker pattern enables horizontal scaling: additional workers can be added without modifying the backend, and different masking models can be deployed in separate containers with independent dependencies.} The frontend uses React and TypeScript to create a single-page application accessible via standard web browsers.
...
The pipeline is demonstrated on two empirical case studies:\footnote{\textbf{Cross-domain application}---The two case studies span different cultural contexts, age groups, and interaction types, demonstrating that the pipeline is not domain-specific but a general-purpose workflow for behavioral observation.}
...
The pipeline produces behavioral metrics that reveal patterns invisible to manual observation:\footnote{\textbf{Discovery, not description}---In the taxonomy of \textcite{grimmer2021machine}, this is \textit{discovery}: the workflow reveals latent structure (movement coordination patterns, cultural differences in interaction dynamics) that was not pre-specified by the researcher but emerges from the computational analysis of frame-by-frame kinematic data.}
