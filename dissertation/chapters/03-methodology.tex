...
Design Science Research is evaluated not only by whether an artifact works but by the \emph{type} of knowledge it produces \parencite{gregor2013positioning}. Following Gregor \& Hevner's taxonomy, this dissertation contributes design knowledge at three levels---each grounded in and tested against the EIT KIC setting:\footnote{\textbf{Why levels matter}---Gregor \& Hevner (2013) distinguish instantiations (does it work?), methods (how to replicate it?), and design principles (what should any such artifact satisfy?). Only the last level is fully transferable beyond the original case.}
