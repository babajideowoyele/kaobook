...
The methodology directly serves the dissertation's research questions: \textbf{RQ1} (What conceptual framework enables theory-guided analysis?) is grounded in DSR's theoretical foundations. \textbf{RQ2} (What methodology enables cross-modal mapping?) is the direct focus of this chapter---we build \RoleBox{} as the methodological answer. \textbf{RQ3} (What role constellation patterns characterize interstitial actors in the EIT ecosystem?) requires the systematic data collection and analytical pipelines we develop here. \textbf{RQ4} (How can design science workflows break down open science barriers while building up infrastructure for cumulative knowledge accumulation?) is addressed through the open-source, reproducible design of our artifacts. The DSR process generates both the artifact and methodological knowledge about multimodal cartography.
\end{chapteroverview}
...
Mapping unknown territory poses a bootstrapping problem. You need methods to create maps, but the adequacy of those methods can only be assessed against the territory they reveal. We could not know whether multimodal cartography would work until we built it; we could not build it without theoretical guidance about what to look for. Design Science Research provides a framework for navigating this productive circularity---legitimizing the iterative construction of tools as a form of inquiry in itself.
\end{insightbox}
...
DSR differs fundamentally from both natural science (explaining what \emph{is}) and interpretive science (understanding what things \emph{mean}). It belongs to the ``sciences of the artificial'' \cite{simon1996sciences}---inquiry into things that \emph{could be} but do not yet exist.
...
\begin{tikzpicture}[scale=0.92]
    ...
    \node[rectangle, rounded corners=8pt, draw=PandaRose, fill=PandaRose!12,
          minimum width=2.4cm, minimum height=2cm, line width=1.5pt, align=center]
          (a4) at (9.6,0) {\scriptsize\textbf{4. Demon-}\\\scriptsize\textbf{stration}\\[3pt]\tiny EIT application\\\tiny Pattern discovery\\\tiny Ch 5--9};

    \node[rectangle, rounded corners=8pt, draw=AccentPurple, fill=AccentPurple!12,
          minimum width=2.4cm, minimum height=2cm, line width=1.5pt, align=center]
          (a5) at (12.8,0) {\scriptsize\textbf{5. Evaluation}\\[6pt]\tiny Technical assessment\\\tiny Theoretical testing\\\tiny Ch 10};

    \node[rectangle, rounded corners=8pt, draw=CoverGold, fill=CoverGold!12,
          minimum width=2.4cm, minimum height=2cm, line width=1.5pt, align=center]
          (a6) at (16,0) {\scriptsize\textbf{6. Communi-}\\\scriptsize\textbf{cation}\\[3pt]\tiny Dissertation\\\tiny Open source\\\tiny Ch 12};
...
Based on theoretical foundations (Chapter~\ref{ch:cas}) and problem analysis, we defined six design objectives that a solution must achieve.\sidenote{\textbf{RQ alignment}---O1--O3 directly enable answering RQ1 (conceptual framework) and RQ3 (empirical role constellation patterns). O4--O6 address RQ2 (cross-modal mapping methodology) and RQ4 (open science infrastructure).}
...
\begin{tikzpicture}[scale=1.1]
    ...
    \node[font=\scriptsize, align=center] at (-5.5,0.4) {\textbf{Websites}\\Self-presentation\\Partner pages};
    \node[font=\scriptsize, align=center, text=Slate] at (-5.5,-0.7) {Ch.~\ref{ch:venture}: Venture\\Positioning};
    ...
    \node[font=\scriptsize, align=center, text=Slate] at (0,3.3) {Ch.~\ref{ch:visual}: Visual\\Registers};
    ...
    \node[font=\scriptsize, align=center] at (5.5,0.4) {\textbf{Discourse + Networks}\\Power dynamics\\Social positioning};
    \node[font=\scriptsize, align=center, text=Slate] at (5.5,-0.7) {Ch.~\ref{ch:power}: Power\\Agency};
...
\subsection{Design Knowledge Contributions}
\labsec{method:design-knowledge}

Design Science Research is evaluated not only by whether an artifact works but by the \emph{type} of knowledge it produces \parencite{gregor2013positioning}. Following Gregor \& Hevner's taxonomy, this dissertation contributes design knowledge at three levels---each grounded in and tested against the EIT KIC setting:\marginnote[0pt]{\footnotesize\textbf{Why levels matter}---Gregor \& Hevner (2013) distinguish instantiations (does it work?), methods (how to replicate it?), and design principles (what should any such artifact satisfy?). Only the last level is fully transferable beyond the original case.}

\begin{description}[leftmargin=0.5cm, itemsep=0.4em]

\item[Instantiation] \RoleBox{} is an operational artifact: working data pipelines, a documented codebase, and a browser-based interface through which researchers or EIT practitioners can explore role constellation maps interactively. Instantiation-level contributions demonstrate \emph{feasibility}---multimodal cartography at ecosystem scale is possible, not merely theoretically conceivable. The interface surfaces the same EIT findings (connector gap, KT imbalance, sociotypes) that emerge from the analytical pipelines, making them accessible to non-technical stakeholders.

\item[Methods] Each modality chapter operationalizes the Maass bridging workflow (Curation $\rightarrow$ Annotation $\rightarrow$ Unsupervised $\rightarrow$ Output) as a concrete, replicable method: which data sources to draw on, which algorithms to apply, which validation steps to document. These methods are not EIT-specific---a researcher studying health innovation or regional clusters can follow the same workflow architecture with different input data. The appendices provide the technical documentation that makes this replication tractable.

\item[Design Principles] The four Meta-Design Requirements (MDRs) are the dissertation's most generalized contribution. Derived inductively from building \RoleBox{} against the EIT case, they are stated as requirements---\emph{what any multimodal cartography system must achieve}---and thus function as design principles transferable beyond this context: MDR-1 (theory-data reconciliation) ensures constructs remain interpretable; MDR-2 (hypothesis-driven data selection) keeps collection purposeful; MDR-3 (computational-human complementarity) balances scale and meaning; MDR-4 (knowledge accumulation infrastructure) enables cumulative progress.

\end{description}

The EIT case is neither merely illustrative nor a scope limitation: it is the \emph{kernel environment} \parencite{hevner2004design} from which all three knowledge levels were generated and evaluated. The 8 KICs, their 1,298 partner organizations, and five data modalities constitute a sufficiently complex, real-world innovation ecosystem to stress-test the artifact, surface unexpected patterns, and warrant the generalizability of the resulting design principles.
...
The empirical component required data collection across six modalities for the eight EIT KICs and their partner ecosystems. Each modality captures different aspects of how intermediary role constellations manifest; together they enable triangulation between claimed, attributed, and enacted identities.
...
From a sociomaterial perspective \cite{orlikowski2008sociomateriality}, this epistemological shift has deeper implications than a change of data source. Designed data passes through the researcher's instruments---interview protocols, survey forms, workshop designs---whose material properties shape what becomes sayable and recordable. Organic data passes through the \emph{platform's} material infrastructure---APIs, database schemas, algorithmic feeds, content management systems---whose affordances shape what becomes visible and traceable.\sidenote{\textbf{Two kinds of mediation}---Designed data is mediated by the researcher's instruments; organic data is mediated by the platform's material infrastructure. Both mediate; neither is transparent. The sociomaterial stance makes this mediation analytically productive rather than merely acknowledged.} Both kinds of data are mediated; neither is transparent. But the \emph{nature} of the mediation differs fundamentally, and this difference carries epistemological consequences.

This sociomaterial awareness explains a methodological choice already made but not yet fully theorised: why we use \emph{five} modalities rather than selecting the ``best'' one. If each platform co-constitutes the roles it makes visible---through specific affordances, constraints, and material architectures---then multimodal cartography is not merely \emph{triangulation} (seeking the ``true'' role through cross-checking) but \textbf{prismatic analysis}. Each modality refracts the phenomenon through different material properties, and the \emph{pattern of refraction} is itself informative. The Maass bridging framework's Curation stage, accordingly, is not only about selecting data sources but about acknowledging the \emph{material conditions} under which each data source was generated---what \textcite{kallinikos2013ambivalent} call the ``ambivalent ontology'' of digital artifacts.
...
News & LexisNexis, MediaCloud & 8,400+ articles & Attributed sociotype \\
Investment & Crunchbase, Dealroom & 2,100 funding rounds & Enacted sociotype \\
Visual & Website screenshots, videos & 15,000+ images, 200 videos & Symbolic positioning \\
...
\begin{insightbox}[Operationalizing Wittmayer et al.'s Role Perspectives]
This multimodal architecture directly operationalizes the three perspectives on roles identified by \textcite{wittmayer2017roles}:
\begin{itemize}[nosep, leftmargin=*]
\item \textbf{Roles as recognizable activities} $\rightarrow$ Website analysis captures what organizations claim to \emph{do}
\item \textbf{Roles as resources} $\rightarrow$ Social media reveals how actors strategically \emph{deploy} role claims
\item \textbf{Roles as boundary objects} $\rightarrow$ News and visual analysis exposes how roles are externally \emph{negotiated}
\end{itemize}
Where Wittmayer et al.'s framework remained qualitative and case-specific, multimodal cartography enables systematic operationalization at population scale---computational methods that preserve theoretical meaning.
\end{insightbox}

% Figure: Six Modalities Visual Overview
\begin{figure*}[htbp]
\begin{center}
\begin{tikzpicture}[scale=0.75]
    % Define colors for each modality
    \definecolor{WikiC}{HTML}{636363}
    \definecolor{WebC}{HTML}{4A90D9}
    \definecolor{NewsC}{HTML}{27AE60}
    \definecolor{CrunchC}{HTML}{E67E22}
    \definecolor{SocialC}{HTML}{1DA1F2}
    \definecolor{VisualC}{HTML}{9B59B6}

    % Central: Role Constellations
    \node[draw, circle, fill=Marigold!30, minimum size=2.5cm, font=\small\bfseries, align=center, line width=1.5pt] (center) at (0,0) {Role\\Constellations};

    % Six modalities in a hexagon
    % Wikipedia (top)
    \node[draw, rounded corners, fill=WikiC!15, minimum width=2.5cm, minimum height=1.4cm, align=center] (wiki) at (0,4.5) {\small\textbf{Wikipedia}\\[-2pt]\tiny encyclopedic\\[-2pt]\tiny positioning};

    % Websites (top right)
    \node[draw, rounded corners, fill=WebC!15, minimum width=2.5cm, minimum height=1.4cm, align=center] (web) at (4,2.3) {\small\textbf{Websites}\\[-2pt]\tiny claimed\\[-2pt]\tiny sociotype};

    % News (bottom right)
    \node[draw, rounded corners, fill=NewsC!15, minimum width=2.5cm, minimum height=1.4cm, align=center] (news) at (4,-2.3) {\small\textbf{News}\\[-2pt]\tiny attributed\\[-2pt]\tiny sociotype};

    % Investment (bottom)
    \node[draw, rounded corners, fill=CrunchC!15, minimum width=2.5cm, minimum height=1.4cm, align=center] (crunch) at (0,-4.5) {\small\textbf{Crunchbase}\\[-2pt]\tiny enacted\\[-2pt]\tiny sociotype};

    % Social (bottom left)
    \node[draw, rounded corners, fill=SocialC!15, minimum width=2.5cm, minimum height=1.4cm, align=center] (social) at (-4,-2.3) {\small\textbf{Social Media}\\[-2pt]\tiny relational\\[-2pt]\tiny sociotype};

    % Visual (top left)
    \node[draw, rounded corners, fill=VisualC!15, minimum width=2.5cm, minimum height=1.4cm, align=center] (visual) at (-4,2.3) {\small\textbf{Visuals}\\[-2pt]\tiny aesthetic\\[-2pt]\tiny sociotype};

    % Arrows to center
    \draw[->, WikiC, line width=1pt] (wiki) -- (center);
    \draw[->, WebC, line width=1pt] (web) -- (center);
    \draw[->, NewsC, line width=1pt] (news) -- (center);
    \draw[->, CrunchC, line width=1pt] (crunch) -- (center);
    \draw[->, SocialC, line width=1pt] (social) -- (center);
    \draw[->, VisualC, line width=1pt] (visual) -- (center);

    % Dimension labels (claimed/attributed/enacted arc)
    \node[font=\tiny\itshape, text=WebC] at (2.8,3.8) {self-presentation};
    \node[font=\tiny\itshape, text=NewsC] at (3.2,-3.8) {external view};
    \node[font=\tiny\itshape, text=CrunchC] at (-0.5,-3.2) {material flows};
    \node[font=\tiny\itshape, text=SocialC] at (-3.2,-3.8) {network ties};
    \node[font=\tiny\itshape, text=VisualC] at (-3.0,3.8) {aesthetic signals};

\end{tikzpicture}
\end{center}
\caption[Six data modalities for multimodal cartography]{Six data modalities feeding into role constellation analysis. Each modality reveals a different sociotype dimension: how actors present themselves (websites), are perceived by others (news), enact roles materially (investment), relate to others (social media), and signal visually (imagery). Wikipedia provides baseline encyclopedic positioning.}
\labfig{six-modalities}
\end{figure*}

\subsection{Sampling Strategy}

Our sampling centers on the eight KICs as intermediaries, but these organizations are embedded within a vast ecosystem of interconnected actors. While we focus analytically on the 8 KICs and their role constellations, understanding their positioning requires situating them within their broader networks:\sidenote{\textbf{Data scope}---8 KICs as core unit, embedded in ecosystem of 6,000+ connected organizations across 6 modalities, 14 years (2010--2024).}

% Using captionof to avoid kaobook caption recursion issue
\begin{center}
\begin{tikzpicture}[scale=0.8]
    % Outer ecosystem cloud
    \draw[PandaBamboo!30, fill=PandaBamboo!5, rounded corners=15pt, dashed]
          (-4.5,-2) rectangle (4.5,3.5);
    \node[font=\tiny, text=PandaBamboo!60, anchor=north east] at (4.3,3.3) {Broader ecosystem: 6,000+ orgs};

    % Core: 8 KICs (center)
    \node[draw, circle, fill=CoverGold!30, minimum size=2cm,
          font=\small\bfseries, align=center, line width=1.5pt, draw=CoverGold] (kics) at (0,1) {8 KICs};

    % Partner ring
    \draw[Marigold, line width=1pt] (0,1) circle (2.2cm);
    \node[font=\tiny, text=Marigold, anchor=west] at (2.3,1) {675 partners};

    % Venture dots scattered around
    \foreach \x/\y in {-3/2.5, -2.5/0.5, -3.5/1.5, 3/2, 2.8/0, 3.2/1.2, -2/2.8, 2/-0.5, -1/3, 1.5/2.8} {
        \fill[PandaBamboo!50] (\x,\y) circle (2pt);
    }
    \node[font=\tiny, text=PandaBamboo!70, anchor=east] at (-3.8,-1.5) {800+ ventures};

    % Some overlap indicators
    \draw[Azure!50, dashed, line width=0.5pt] (-1.5,0) -- (-3,0.5);
    \draw[Azure!50, dashed, line width=0.5pt] (1.2,2.2) -- (2,2.8);

    % Label
    \node[font=\scriptsize, align=center] at (0,-1.5) {\textit{Overlapping partnerships,}\\[-2pt]\textit{shared ventures, network ties}};
\end{tikzpicture}
\captionof{figure}{Concentric sampling with ecosystem context. The 8 KICs form the core unit of analysis, embedded within overlapping networks of partners and ventures. Dashed lines indicate cross-KIC connections.}
\labfig{sampling}
\end{center}

\begin{description}[leftmargin=0.5cm, itemsep=0.3em]
\item[Core -- 8 KICs] The eight EIT Knowledge and Innovation Communities are the primary unit of analysis.\sidenote{\textbf{Core focus}---Each KIC is analyzed as an intermediary with its own role constellation, but also in relation to other KICs.} For each KIC, we collect comprehensive data across all five modalities to map its role constellation. Importantly, the KICs share some partners and operate in overlapping domains---their role constellations are not isolated but interconnected.

\item[Inner -- Partners] Direct KIC partners (universities, research institutes, corporations) provide context for how KICs position themselves within their ecosystems. We identified 675 unique partner organizations, but many partner with multiple KICs---creating overlap that shapes how role constellations interact.

\item[Outer -- Ventures] Startups and ventures supported by KIC accelerators reveal how KICs enact their intermediary roles. The 800+ ventures connect outward to thousands of investors, customers, and collaborators---embedding the EIT ecosystem within broader innovation networks.

\item[Broader Context] The 8 KICs are connected to over 6,000 organizations when tracing partner relationships, funding networks, and news co-mentions. While we focus analytically on the intermediaries themselves, this broader context is essential for understanding their relational positioning.
\end{description}

\subsection{Collection Workflows by Modality}

Each modality required specialized collection procedures, summarized in Table~\ref{tab:collection-workflows} and detailed in the data modality appendices.\sidenote{\textbf{Full documentation}---Appendices D--H provide complete workflow specifications, code references, and data dictionaries for each modality.}

% Collection workflows table - compact horizontal layout
\begin{table*}[htbp]
\centering
\small
\caption{Data collection workflows by modality}
\label{tab:collection-workflows}
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Modality} & \textbf{Pipeline Steps} & \textbf{Tools} & \textbf{Yield} & \textbf{Duration} \\
\midrule
Websites & URL $\to$ crawl $\to$ screenshot $\to$ extract & Scrapy, Playwright & 675 sites & 3 weeks \\
News & Query $\to$ API $\to$ dedupe $\to$ filter & LexisNexis, GDELT & 8,400+ articles & 2 months \\
Investment & Match $\to$ funding $\to$ expand & Crunchbase, Dealroom & 2,100 rounds & 1 month \\
Visual & Screenshot $\to$ extract $\to$ catalogue & Playwright, OpenCV & 15K+ images & 2 weeks \\
Social & Account $\to$ timeline $\to$ network & Twitter API v2 & 1.45M tweets & 6 weeks \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{tikzpicture}[scale=1.1, every node/.style={font=\small}]
    ...
    \node[draw, rounded corners=4pt, fill=Slate!10, minimum width=1.8cm, minimum height=1cm, align=center] (s1) at (0,0) {Source\\ID};
    \node[draw, rounded corners=4pt, fill=Azure!15, minimum width=1.8cm, minimum height=1cm, align=center] (s2) at (3,0) {API/\\Crawl};
    \node[draw, rounded corners=4pt, fill=PandaBamboo!15, minimum width=1.8cm, minimum height=1cm, align=center] (s3) at (6,0) {Clean \&\\Filter};
    \node[draw, rounded corners=4pt, fill=Marigold!15, minimum width=1.8cm, minimum height=1cm, align=center] (s4) at (9,0) {Extract\\Features};
    \node[draw, rounded corners=4pt, fill=CoverGold!15, minimum width=1.8cm, minimum height=1cm, align=center] (s5) at (12,0) {Store \&\\Index};

    \draw[->, very thick, Slate] (s1) -- (s2);
    \draw[->, very thick, Slate] (s2) -- (s3);
    \draw[->, very thick, Slate] (s3) -- (s4);
    \draw[->, very thick, Slate] (s4) -- (s5);

    % Labels
    \node[font=\small\itshape, text=Slate] at (6,-1.3) {Common workflow pattern across all modalities};
\end{tikzpicture}
\captionof{figure}{Generalized collection workflow. Each modality instantiates this pattern with modality-specific tools and validation steps.}
\labfig{collection-workflow}
\end{center}
...
Figure~\ref{fig:bridging-framework} adapts the Maass et al. \citeyear{maass2018data} framework to show how multimodal cartography bridges computational pattern discovery with transitions theory development.

% Maass-style bridging framework figure
\begin{center}
\begin{tikzpicture}[scale=0.9]
    % Left side: Multimodal Data
    \node[draw, rounded corners=12pt, fill=Azure!15, minimum width=2.8cm, minimum height=2.2cm,
          line width=1.2pt, align=center] (data) at (-5,2.5) {\textbf{Multimodal}\\[2pt]\textbf{Data}};

    % Right side: Transitions Theory
    \node[draw, rounded corners=12pt, fill=Marigold!15, minimum width=2.8cm, minimum height=2.2cm,
          line width=1.2pt, align=center] (theory) at (5,2.5) {\textbf{Transitions}\\[2pt]\textbf{Theory}};

    % Center: RoleBox/Multimodal Cartography
    \node[draw, rounded corners=8pt, fill=CoverGold!25, minimum width=3.2cm, minimum height=1.6cm,
          line width=1.5pt, align=center, font=\small\bfseries] (bridge) at (0,2.5) {Multimodal\\Cartography};

    % Bottom left: Data-driven research
    \node[draw, rounded corners=8pt, fill=Azure!8, minimum width=2.6cm, minimum height=1.4cm,
          line width=0.8pt, align=center, font=\small] (datadriven) at (-5,0) {Data-Driven\\Research};

    % Bottom right: Theory-driven research
    \node[draw, rounded corners=8pt, fill=Marigold!8, minimum width=2.6cm, minimum height=1.4cm,
          line width=0.8pt, align=center, font=\small] (theorydriven) at (5,0) {Theory-Driven\\Research};

    % Top arrow: Patterns to theory
    \draw[->, line width=1.5pt, Azure!70] (data.east) -- (bridge.west)
          node[midway, above, font=\scriptsize\itshape] {patterns};
    \draw[->, line width=1.5pt, Marigold!70] (bridge.east) -- (theory.west)
          node[midway, above, font=\scriptsize\itshape] {constructs};

    % Bottom arrow: Theory to data requirements
    \draw[->, line width=1.5pt, Marigold!70] (theory.west) to[out=200, in=20]
          node[midway, below, font=\scriptsize\itshape, yshift=-3pt] {hypotheses} (bridge.east);
    \draw[->, line width=1.5pt, Azure!70] (bridge.west) to[out=160, in=340]
          node[midway, below, font=\scriptsize\itshape, yshift=-3pt] {data needs} (data.east);

    % Vertical connections
    \draw[dashed, Slate!50, line width=0.8pt] (data) -- (datadriven);
    \draw[dashed, Slate!50, line width=0.8pt] (theory) -- (theorydriven);
    \draw[dashed, CoverGold!70, line width=1pt] (bridge) -- (0,0)
          node[below, font=\scriptsize, align=center] {\textit{bridging}\\[-2pt]\textit{knowledge}};

    % Labels for the two paths
    \node[font=\tiny, text=Azure!80, anchor=west] at (-4.5,4.2) {\textbf{Path 1:} Patterns $\rightarrow$ Theory};
    \node[font=\tiny, text=Marigold!80, anchor=east] at (4.5,0.8) {\textbf{Path 2:} Theory $\rightarrow$ Data};

\end{tikzpicture}
\captionof{figure}[Bridging data-driven and theory-driven research]{Multimodal cartography as a bridge between data-driven and theory-driven research (adapted from Maass et al. 2018). \textbf{Path 1}: Computational patterns from multimodal data are abstracted into theoretical constructs about role constellations. \textbf{Path 2}: Transitions theory guides data requirements and analytical choices. The \RoleBox{} toolkit embodies the bridging knowledge that enables both pathways.}
\label{fig:bridging-framework}
\end{center}

Table~\ref{tab:challenges-resolution} shows how our methodology addresses each of the four challenges Maass et al. identify for research at the intersection of data-driven and theory-driven perspectives.
...
\addlinespace
\textbf{C3: Solving} problems unsolvable from single perspective & Problem refinement \& decomposition & RQ2 (cross-modal mapping) requires both computational comparison and theoretical interpretation \\
\addlinespace
...
\begin{tikzpicture}[scale=1.0, every node/.style={font=\small}]
    % Pipeline stages
    \node[draw, rounded corners, fill=Azure!15, minimum width=2cm, minimum height=1cm]
          (t1) at (0,0) {Raw Text};
    \node[draw, rounded corners, fill=Azure!15, minimum width=2cm, minimum height=1cm]
          (t2) at (3,0) {Clean Text};
    \node[draw, rounded corners, fill=PandaBamboo!15, minimum width=2cm, minimum height=1cm]
          (t3) at (6,0) {Tokens};
    \node[draw, rounded corners, fill=PandaBamboo!15, minimum width=2cm, minimum height=1cm]
          (t4) at (9,0) {Entities};
    \node[draw, rounded corners, fill=Marigold!15, minimum width=2cm, minimum height=1cm]
          (t5) at (12,0) {Topics};
    \node[draw, rounded corners, fill=CoverGold!15, minimum width=2.2cm, minimum height=1cm]
          (t6) at (15.2,0) {Embeddings};

    % Arrows with labels
    \draw[->, thick] (t1) -- node[above, font=\footnotesize] {preprocess} (t2);
    \draw[->, thick] (t2) -- node[above, font=\footnotesize] {tokenize} (t3);
    \draw[->, thick] (t3) -- node[above, font=\footnotesize] {NER} (t4);
    \draw[->, thick] (t4) -- node[above, font=\footnotesize] {LDA/BERTopic} (t5);
    \draw[->, thick] (t5) -- node[above, font=\footnotesize] {encode} (t6);
\end{tikzpicture}
\captionof{figure}{Text analysis pipeline from raw text through embeddings.}
\labfig{text-pipeline}
\end{center}

The pipeline begins with \textbf{preprocessing}---language detection, encoding normalization, and boilerplate removal \cite{bird2009nlp}---to produce clean text suitable for analysis.\sidenote{\textbf{For non-specialists}---These NLP (natural language processing) steps transform messy web text into structured data. Think of it as cleaning and organizing documents before filing them.} \textbf{Tokenization} using spaCy \cite{honnibal2020spacy} segments text into sentences and words. \textbf{Named Entity Recognition} extracts organizations, persons, locations, and technologies \cite{ner2018survey}---the actors and artifacts that populate role constellations. \textbf{Topic modeling} uses BERTopic \cite{grootendorst2022bertopic} for semantic clustering and guided LDA \cite{blei2003lda} for theory-informed topic extraction, revealing the thematic content of organizational self-presentation.\sidenote{\textbf{LLMs for topic labeling}---Large language models can assist with interpreting topic clusters by suggesting coherent labels. However, domain expertise remains crucial for mapping computational topics to theoretical constructs like role categories. We used LLM suggestions as starting points, not final answers.} Finally, \textbf{embedding} via Sentence-BERT \cite{reimers2019sentence,devlin2019bert} produces dense vector representations that enable similarity computation across organizations---the foundation for mapping positions in semantic space.

\begin{insightbox}[Knowledge Graph Extraction with Local LLMs]
For any text corpus, \textbf{kg-gen} enables automated extraction of knowledge graphs---structured networks of entities and their relationships. Using local LLMs (Llama 3.2, Mistral via Ollama), the pipeline processes documents and extracts subject-predicate-object triples without requiring cloud API access. The resulting graphs can be visualized interactively, enabling rapid exploration of actor relationships, organizational connections, and conceptual structures. Each \texttt{rolebox-*} module includes a \texttt{ui/} folder with pre-built HTML visualizations; the extraction scripts in \texttt{scripts/} can be adapted to new corpora. See Appendices C and E for example outputs from Wikipedia and news data respectively.
\end{insightbox}
...
Our research involves organizational data from public sources.\sidenote{\textbf{Ethics principles}---Public data only, organizational focus, no individual identification, platform compliance, open science.} We addressed ethical considerations proactively:
...
\item \textbf{Demonstrative scope}: The analytical pipelines are \emph{proof-of-concept} demonstrations of what multimodal cartography can achieve, not exhaustive mappings. Findings should be interpreted as exploratory patterns warranting further investigation rather than conclusive evidence.
...
\item \textbf{Interpretive judgment}: Despite systematic methods, role identification involves researcher judgment---in codebook development, threshold setting, and pattern interpretation.\marginnote{\textbf{Model-assisted annotation}---We explored using large language models (LLMs) to assist with coding and classification tasks. While promising for consistency, these tools remain early-stage: they can suggest categories but struggle with domain-specific nuance. Human judgment remains essential for validating model outputs against theoretical constructs. The future likely involves human-in-the-loop workflows rather than full automation.}
\end{enumerate}
\end{criticalbox}
