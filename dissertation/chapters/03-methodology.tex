...
\chapter{Methodology: Design Science Research for Cartographic Innovation}
\labch{method}
\margintoc
...
\sidenote{\textbf{DSR Origins}---Developed in information systems by Simon (1996), Hevner et al. (2004), and Peffers et al. (2007). Now applied across design-oriented disciplines including engineering, architecture, and increasingly, social science.}

\subsection{The Logic of Design Science}

DSR differs fundamentally from both natural science (explaining what \emph{is}) and interpretive science (understanding what things \emph{mean}). It belongs to the ``sciences of the artificial''---inquiry into things that \emph{could be} but do not yet exist.
...
We followed Peffers et al.'s (2007) six-activity DSR process model, adapted for our context. The process is explicitly iterative---we cycled through activities multiple times over four years, with each cycle refining both the artifact and our understanding of the problem.

\sidenote{\faClock\ \textbf{Timeline}---2021: Problem identification. 2022: First design iteration. 2023: Core development, EIT data collection. 2024: Integration, evaluation, refinement. 2025: Dissertation completion.}
...
\sidenote{\faSearch\ \textbf{Problem sources}---Academic literature, conference discussions, practitioner needs, research obstacles.}
...
Based on theoretical foundations (Chapter~\ref{ch:theory}) and problem analysis, we defined six design objectives that a solution must achieve:
...
This was the core creative work---designing architecture, selecting technologies, implementing pipelines, and iterating through failures. Chapter~\ref{ch:xray} details the resulting artifact; here we describe the design \emph{process}.

\sidenote{\faWrench\ \textbf{Design iterations}---v0.1: Single-modality proof of concept. v0.5: Multi-modality pipelines. v1.0: Full integration layer. v1.5: Refinement based on EIT application.}
...
Demonstration applies the artifact to a real problem, showing that it works. Chapters~\ref{ch:websites}--\ref{ch:integration} demonstrate the \RoleBox toolkit on the EIT ecosystem:

\sidenote{\faFlask\ \textbf{Demonstration scope}---6,487 organizations, 8 EIT KICs, 5 data modalities, 2019--2024 timeframe, 218,000+ records.}

\begin{itemize}[leftmargin=*, itemsep=0.2em]
\item \textbf{Website analysis} (Chapter~\ref{ch:websites}): Self-presented identities and claimed roles
\item \textbf{News analysis} (Chapter~\ref{ch:news}): Media portrayal and attributed roles
\item \textbf{Network analysis} (Chapter~\ref{ch:networks}): Relational positions and structural roles
\item \textbf{Investment analysis} (Chapter~\ref{ch:investment}): Resource flows and financial positioning
\item \textbf{Visual analysis} (Chapter~\ref{ch:visual}): Aesthetic positioning and symbolic roles
\item \textbf{Integration} (Chapter~\ref{ch:integration}): Cross-modal synthesis and validation
\end{itemize}
...
Chapter~\ref{ch:evaluation} presents detailed evaluation results. The key question is not ``Is it perfect?'' but ``Does it advance our ability to map role constellations beyond what was previously possible?''

\subsection{Activity 6: Communication}

Communication shares the artifact and generated knowledge with relevant communities:

\sidenote{\faShare\ \textbf{Communication channels}---This dissertation, open-source code (GitHub), archived datasets (Zenodo), conference presentations, journal articles.}
...
The artifact we built is the \textbf{\RoleBox toolkit}---an integrated system for multimodal cartography of role constellations. Chapter~\ref{ch:toolkit} provides detailed technical documentation; here we present the architectural overview.
...
\sidenote{\faDatabase\ \textbf{Data scale}---6,487 organizations, 218,000+ records, 5 modalities, 5+ years coverage.}

\subsection*{Population Definition and Sampling}

Our target population is organizations connected to the EIT Knowledge and Innovation Communities. We employed a multi-stage sampling strategy:
...
\sidenote{\faFilter\ \textbf{Inclusion criteria}---Listed as KIC partner, OR mentioned by 3+ partners, OR news coverage with KIC, OR investment from KIC fund; AND present in 2+ modalities.}

\begin{description}[leftmargin=0.5cm, itemsep=0.3em]
\item[Stage 1 -- Seed] We scraped official partner directories from all eight KIC websites, yielding 2,420 confirmed partner organizations.
...
\item \textbf{Rate limiting}: Respectful crawling with delays; honor robots.txt
\end{enumerate}

\sidenote{\faGlobe\ \textbf{Website workflow}---Tool: Scrapy + Playwright. Pages/org: $\sim$2 (median). Success rate: 94\%. Duration: 3 weeks.}
...
\item \textbf{Full-text retrieval}: Obtain complete article text where available
\end{enumerate}

\sidenote{\faNewspaper\ \textbf{News workflow}---Sources: 3 APIs. Articles/org: $\sim$5 (median). Languages: EN (primary). Duration: 2 months.}
...
\item \textbf{Validation}: Cross-check funding data across sources
\end{enumerate}

\sidenote{\faChartLine\ \textbf{Investment workflow}---Sources: Crunchbase, Dealroom. Match rate: 20\% (startups). Funding rounds: 2,100. Duration: 1 month.}
...
\item \textbf{Network extraction}: Identify mentions, replies, quote tweets
\end{enumerate}

\sidenote{\faTwitter\ \textbf{Social workflow}---Platform: Twitter/X. Account match: 37\%. Tweets/account: $\sim$65 (median). Note: API access limited post-2023.}
...
\item[Topic Modeling] BERTopic for semantic clustering; guided LDA for theory-informed topics
\item[Embedding] Sentence-BERT for dense vector representations enabling similarity computation
\end{description}
...
Analysis employs standard network metrics (degree, betweenness, eigenvector centrality) plus community detection (Louvain, Infomap) and structural equivalence measures.
...
\item[CNN features] Pre-trained ResNet features for visual similarity
...
Following Hevner et al. (2004), we explicitly address seven DSR guidelines:
...
\sidenote{\faCheckCircle\ \textbf{Seven guidelines}---Hevner et al. (2004) established canonical DSR guidelines. We address each explicitly.}

\subsection{Validity Considerations}

We consider standard validity threats:
...
Our research involves organizational data from public sources. We addressed ethical considerations proactively:

\sidenote{\faBalanceScale\ \textbf{Ethics principles}---Public data only, organizational focus, no individual identification, platform compliance, open science.}
...
\sidenote{\faHeart\ \textbf{Honest limitations}---Acknowledging limitations strengthens rather than weakens research. Others can address what we could not.}

These limitations do not invalidate the research---they \emph{bound} it. We are building a toolkit and demonstrating its potential, not claiming definitive answers. The open infrastructure enables others to extend, adapt, and apply the approach in contexts where our limitations matter less.
...
The methodology enables us to build cartographic infrastructure while simultaneously using that infrastructure to map the EIT ecosystem. The next chapter details the \RoleBox toolkit architecture and implementation.
\end{summarybox}

\sidenote{\faArrowRight\ \textit{Next:} Chapter~\ref{ch:toolkit}: The \RoleBox{} Toolkit---detailed technical documentation of components, pipelines, and integration methods.}
