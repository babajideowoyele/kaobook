...
This chapter directly addresses \textbf{RQ1} (What conceptual framework enables theory-guided but empirically open analysis of role constellations?) by developing CAS as the theoretical foundation, and establishes fifteen propositions that structure the empirical inquiry of \textbf{RQ3} (What role constellation patterns characterize interstitial actors in the EIT ecosystem?).
\end{chapteroverview}
...
% Figure: Six CAS mechanisms overview (2×3 grid)
\begin{center}
\begin{tikzpicture}[
    panel/.style={draw=Slate!40, rounded corners=4pt, line width=0.8pt, fill=Slate!5,
                  minimum width=5.3cm, minimum height=3.8cm},
    ptitle/.style={font=\small\bfseries, text=PandaCharcoal},
]
...
% Panel 1: Emergence  (col 1, row 1)
\begin{scope}[xshift=0cm, yshift=0cm]
    \node[panel] at (0,0) {};
    \node[ptitle] at (0, 1.55) {1. Emergence};
    % Micro agents at bottom
    \foreach \x in {-1.6,-0.96,-0.32,0.32,0.96,1.6} {
        \node[draw=Slate!60, circle, fill=Slate!30, inner sep=2.8pt] at (\x, -1.1) {};
    }
    % Upward arrow
    \draw[->, thick, PandaCharcoal!70] (0, -0.55) -- (0, 0.3);
    \node[font=\tiny, text=PandaCharcoal!70, anchor=west] at (0.08, -0.15) {emergent};
    % Macro hexagon at top
    \node[draw=Azure, fill=Azure!25, regular polygon, regular polygon sides=6,
          minimum size=1.0cm, line width=1pt] at (0, 0.9) {};
    \node[font=\tiny\bfseries, text=Azure] at (0, 0.9) {Const.};
\end{scope}

% Panel 2: Self-Organization  (col 2, row 1)
\begin{scope}[xshift=5.8cm, yshift=0cm]
    \node[panel] at (0,0) {};
    \node[ptitle] at (0, 1.55) {2. Self-Organization};
    % Group A (Azure)
    \node[draw=Azure!70, circle, fill=Azure!35, inner sep=3pt] (sa1) at (-1.3, 0.6) {};
    \node[draw=Azure!70, circle, fill=Azure!35, inner sep=3pt] (sa2) at (-0.7, 0.6) {};
    \node[draw=Azure!70, circle, fill=Azure!35, inner sep=3pt] (sa3) at (-1.0, 0.0) {};
    \draw[thick, Azure!60] (sa1)--(sa2)--(sa3)--(sa1);
    % Group B (Marigold)
    \node[draw=Marigold!70, circle, fill=Marigold!35, inner sep=3pt] (sb1) at (0.7, 0.5) {};
    \node[draw=Marigold!70, circle, fill=Marigold!35, inner sep=3pt] (sb2) at (1.3, 0.0) {};
    \draw[thick, Marigold!60] (sb1)--(sb2);
    % Group C (PandaBamboo)
    \node[draw=PandaBamboo!70, circle, fill=PandaBamboo!35, inner sep=3pt] (sc1) at (-0.5, -0.9) {};
    \node[draw=PandaBamboo!70, circle, fill=PandaBamboo!35, inner sep=3pt] (sc2) at (0.2, -1.1) {};
    \node[draw=PandaBamboo!70, circle, fill=PandaBamboo!35, inner sep=3pt] (sc3) at (0.9, -0.9) {};
    \draw[thick, PandaBamboo!60] (sc1)--(sc2)--(sc3);
    % Dashed inter-group lines
    \draw[dashed, Slate!40, thin] (sa2)--(sb1);
    \draw[dashed, Slate!40, thin] (sb2)--(sc3);
    % "No center" label
    \node[font=\tiny, text=Slate!70] at (0, -1.5) {no central coordinator};
\end{scope}

% Panel 3: Co-evolution  (col 3, row 1)
\begin{scope}[xshift=11.6cm, yshift=0cm]
    \node[panel] at (0,0) {};
    \node[ptitle] at (0, 1.55) {3. Co-evolution};
    % Org wavy arrow (top lane)
    \draw[->, thick, Azure] (-1.8, 0.55)
        .. controls (-1.0, 0.85) and (-0.2, 0.25) ..
        (0.6, 0.55)
        .. controls (1.0, 0.75) and (1.4, 0.45) .. (1.8, 0.55);
    \node[font=\tiny\bfseries, text=Azure] at (-2.05, 0.55) {Org};
    % Env wavy arrow (bottom lane)
    \draw[->, thick, Marigold] (-1.8, -0.55)
        .. controls (-1.0, -0.25) and (-0.2, -0.85) ..
        (0.6, -0.55)
        .. controls (1.0, -0.35) and (1.4, -0.65) .. (1.8, -0.55);
    \node[font=\tiny\bfseries, text=Marigold] at (-2.05, -0.55) {Env};
    % Bidirectional coupling arrows at 3 points
    \foreach \x in {-0.6, 0.4, 1.4} {
        \draw[<->, thin, PandaCharcoal!60] (\x, 0.35) -- (\x, -0.35);
    }
    \node[font=\tiny, text=Slate!70] at (0, -1.5) {mutual adaptation};
\end{scope}
...
% Panel 4: Non-linearity  (col 1, row 2)
\begin{scope}[xshift=0cm, yshift=-4.6cm]
    \node[panel] at (0,0) {};
    \node[ptitle] at (0, 1.55) {4. Non-linearity};
    % Axes
    \draw[->, Slate!60, thin] (-1.9, -1.0) -- (1.9, -1.0)
        node[right, font=\tiny, text=Slate!70] {Input};
    \draw[->, Slate!60, thin] (-1.9, -1.0) -- (-1.9, 1.2)
        node[above, font=\tiny, text=Slate!70] {Output};
    % S-curve / step: flat, jump, flat
    \draw[thick, Azure, line cap=round]
        (-1.8, -0.7) -- (-0.3, -0.7)
        .. controls (0.0, -0.7) and (0.1, 0.6) ..
        (0.3, 0.7) -- (1.8, 0.7);
    % Tipping point dashed line
    \draw[dashed, AccentCoral, thin] (0.0, -1.0) -- (0.0, 1.2);
    \node[font=\tiny, text=AccentCoral, anchor=north, rotate=90] at (0.15, -0.6) {tipping pt};
\end{scope}

% Panel 5: Feedback Loops  (col 2, row 2)
\begin{scope}[xshift=5.8cm, yshift=-4.6cm]
    \node[panel] at (0,0) {};
    \node[ptitle] at (0, 1.55) {5. Feedback Loops};
    % Reinforcing loop (left)
    \node[draw=PandaBamboo!70, rounded corners=3pt, fill=PandaBamboo!20,
          font=\tiny, minimum width=1.1cm, inner sep=2pt] (fr1) at (-1.1, 0.5)  {Success};
    \node[draw=PandaBamboo!70, rounded corners=3pt, fill=PandaBamboo!20,
          font=\tiny, minimum width=1.1cm, inner sep=2pt] (fr2) at (-1.1, -0.4) {Resources};
    \draw[->, thick, PandaBamboo] (fr1.south) -- (fr2.north);
    \draw[->, thick, PandaBamboo] (fr2.west) to[out=180,in=180] (fr1.west);
    \node[font=\tiny\bfseries, text=PandaBamboo] at (-1.1, -1.1) {+ (reinforcing)};
    % Balancing loop (right)
    \node[draw=AccentCoral!70, rounded corners=3pt, fill=AccentCoral!15,
          font=\tiny, minimum width=1.1cm, inner sep=2pt] (fb1) at (1.1, 0.5)  {Growth};
    \node[draw=AccentCoral!70, rounded corners=3pt, fill=AccentCoral!15,
          font=\tiny, minimum width=1.1cm, inner sep=2pt] (fb2) at (1.1, -0.4) {Cost};
    \draw[->, thick, AccentCoral] (fb1.south) -- (fb2.north);
    \draw[->, thick, AccentCoral] (fb2.east) to[out=0,in=0] (fb1.east);
    \node[font=\tiny\bfseries, text=AccentCoral] at (1.1, -1.1) {- (balancing)};
\end{scope}

% Panel 6: Fitness Landscapes  (col 3, row 2)
\begin{scope}[xshift=11.6cm, yshift=-4.6cm]
    \node[panel] at (0,0) {};
    \node[ptitle] at (0, 1.55) {6. Fitness Landscapes};
    % Landscape curve
    \draw[thick, fill=Slate!15]
        (-1.8, -0.9)
        .. controls (-1.2, 0.3) and (-0.8, 0.5) .. (-0.5, 0.3)
        .. controls (-0.2, 0.1) and (0.1, -0.5) .. (0.4, -0.3)
        .. controls (0.7, -0.1) and (1.0, 0.9) .. (1.3, 0.95)
        .. controls (1.5, 0.9) and (1.7, 0.2) .. (1.8, -0.3)
        -- (1.8, -0.9) -- (-1.8, -0.9) -- cycle;
    % Axis labels
    \node[font=\tiny, text=Slate!70] at (0, -1.35) {Role dimensions};
    \node[font=\tiny, text=Slate!70, rotate=90] at (-2.1, 0) {Fitness};
    % Peak labels
    \node[font=\tiny, text=Slate] at (-0.5, 0.7) {local};
    \node[font=\tiny, text=Slate] at (1.3, 1.3) {global};
    % Valley label
    \node[font=\tiny, text=Slate!60] at (0.3, -0.6) {valley};
    % Dots on peaks
    \node[draw=Azure!80, circle, fill=Azure!50, inner sep=2.5pt] at (-0.5, 0.42) {};
    \node[draw=Marigold!80, circle, fill=Marigold!60, inner sep=2.5pt] at (1.3, 1.05) {};
\end{scope}

\end{tikzpicture}

\captionof{figure}{Six CAS mechanisms for role constellation dynamics. Each mechanism generates
    distinct observable patterns in multimodal cartography data.}
\labfig{cas:mechanisms:overview}
\end{center}
