...
This chapter introduces \textbf{multimodal cartography} as a response: a workflow-centric Design Science approach to mapping role constellations by building computational workflows that integrate multiple data modalities. It establishes the problem, makes the case for treating methodological integration as an artifact-building challenge, introduces \RoleBox{} as the \emph{demonstrative} platform implementing these workflows (including data pipelines, interactive UIs, and annotation protocols), and maps the dissertation's journey ahead.
\end{chapteroverview}
...
Intermediary organizations occupy a special place in transitions \cite{kivimaa2019innovation,kanda2020conceptualising,vanlente2020intermediaries}.\sidenote{\textbf{Intermediaries}---Organizations bridging innovators and regimes, translating between logics, navigating barriers.} Positioned between innovators and regimes, between sectors, between policy and practice, they perform critical bridging functions: facilitating connections, translating between logics, providing resources, conferring legitimacy, and helping innovations navigate institutional barriers.
...
The ``multimodal'' dimension is essential.\sidenote{\textbf{Why multimodal?}---Organizations \emph{claim} roles on websites, \emph{perform} them on Twitter, and are \emph{perceived} differently in media.}\marginnote{\textbf{Exploratory ethos}---Rather than imposing predetermined categories, we approached each data modality with openness: \emph{what can this lens reveal about role constellations?} This data-driven exploration allowed patterns to emerge inductively---sociotypes, gaps, imbalances---that theory-first approaches would have missed.} Roles are not singular phenomena observable through any single lens. An intermediary may claim a convening role on its website, perform a brokering role through its Twitter interactions, and be perceived as an advocate in news coverage. These are not contradictions to be resolved but facets to be mapped. Single-modality analysis\textemdash whether text-only, network-only, or interview-only\textemdash inevitably misses dimensions that other modalities would reveal.
...
Design Science Research (DSR) provides a systematic framework for this kind of methodological innovation.\sidenote{\textbf{Design Science Research}---Paradigm treating artifact creation and evaluation as scientific contribution.} \sidecite{hevner2004design} Originating in information systems research \cite{march1995design}, DSR treats the creation and evaluation of novel artifacts as legitimate scientific contribution \cite{peffers2007dsr}. Unlike explanatory research that seeks to understand existing phenomena, or critical research that interrogates power and assumptions, design research asks: \emph{what new capabilities become possible through systematic artifact creation?}

Yet DSR remains largely uncharted territory in sustainability transitions research.\sidenote{\textbf{The artifact gap}---Transitions research excels at theory but rarely produces reusable tools or datasets.} The field has produced sophisticated theoretical frameworks\textemdash the Multi-Level Perspective, Strategic Niche Management, Transition Management\textemdash but has devoted far less attention to the \emph{artifacts} that enable empirical analysis.
...
This absence is consequential. Without such artifacts, each research team starts from scratch. Findings cannot be replicated or extended. Methodological innovations remain locked in individual projects rather than accumulating as shared infrastructure. The field advances through theoretical refinement but lacks the material foundations for cumulative empirical progress.\sidenote{\textbf{De Haan et al. (2019)}---``Not only is there no such database but moreover it seems contrary to the way data are used in transitions research.''}

This dissertation imports the Design Science paradigm from information systems to address this gap.\sidenote{\textbf{Importing from IS}---This dissertation adapts DSR from information systems for social science methodology.} The contribution is not merely to \emph{use} computational methods but to \emph{design, document, and share} the artifacts that make multimodal cartography accessible and reproducible.
...
\item[MDR-4: Knowledge Accumulation Infrastructure.] Reusable artifacts---open-source code, documented data, transparent procedures---enabling cumulative progress.\sidenote{\textbf{Full MDR treatment}---Chapter~3 develops these requirements in detail and shows how each is operationalized throughout the dissertation.}
\end{description}
...
\textbf{Methodological contributions} demonstrate that Design Science Research can productively guide social science methodology development.\sidenote{DSR framework; four MDRs; multimodal organic data; integration protocols.} The four Meta-Design Requirements provide a transferable framework for bridging data-driven and theory-driven approaches. Critically, this dissertation responds to calls for ``data-driven transitions research'' by demonstrating how \emph{multimodal organic data}---digital traces generated independently of research objectives, integrated across five modalities---can substitute for the \emph{designed data} (interviews, surveys) that dominates the field. This enables population-scale analysis while preserving theoretical meaning, venturing into methodological territory that remains largely uncharted in transitions studies. Beyond artifacts, the dissertation articulates \emph{design knowledge}---seven design principles, five design patterns, and practitioner guidelines that transfer to other multimodal research contexts.

\textbf{Empirical contributions} demonstrate the platform's analytical potential through exploratory application to eight intermediation domains.\sidenote{2,192 orgs, 8 communities, 14 years, 6 modalities.} Key patterns include the \emph{connector gap} (aspiration-reality divergence), \emph{knowledge triangle imbalance} (business dominance), and \emph{five recurring sociotypes} that emerge across domains.

\textbf{Infrastructural contributions} create the foundations for cumulative knowledge development.\sidenote{Open-source \RoleBox{}; interactive UIs; annotation protocols; CARTALOG; \RoleSim{}; replication packages.} \RoleBox{} provides an open-source platform with six documented computational workflows, each with modality-specific data pipelines, interactive visualization UIs, and systematic annotation guidelines. \textbf{CARTALOG}, the master organizational catalog, links 8,600+ organizations across all data modalities---enabling researchers to trace how the same organization is portrayed across websites, social media, news, and visual materials. This workflow-based infrastructure supports both computational queries and theory-driven exploration, addressing calls for ``data-driven transitions research'' with reusable artifacts.
...
\textbf{Methodologically}, the heavy reliance on digital trace data privileges what is observable online.\sidenote{Digital traces only; offline activities not captured.} Organizations with limited digital presence, interactions through non-digital channels, and dimensions leaving few digital traces are underrepresented or invisible.
...
The journey begins with an intermezzo introducing the EIT ecosystem, then proceeds to Chapter~\ref{ch:background}, where we survey the territory itself: sustainability transitions, the actors who shape them, and the roles those actors play.\sidenote{\textit{Next}: \textbf{Intermezzo}---The EIT Ecosystem introduces the eight Knowledge and Innovation Communities serving as our empirical laboratory.}
