...
\begin{tikzpicture}[scale=1.1,
    ...
    \node[draw, rounded corners=5pt, fill=PandaCharcoal!25, minimum width=14cm, minimum height=1.8cm, align=center, font=\normalsize] (main) at (0,4.5) {\textbf{Main Research Question}\\[0.2em]\small How can role constellations be studied empirically at scale---from typologies to\\multimodal cartography---and what are the implications for navigating transitions?};
    ...
    \node[font=\scriptsize, text=Slate, align=center, text width=3.5cm] at (1.8,-1) {What role patterns emerge in the EIT ecosystem?};

    \node[font=\small\bfseries, text=AccentPurple] at (5.5,-0.2) {Implications};
    \node[font=\scriptsize, text=Slate, align=center, text width=3.5cm] at (5.5,-1) {What do patterns imply for navigating transitions?};
    ...
    \node[font=\scriptsize\itshape, text=Slate] at (1.8,-1.8) {Ch.~5--9};
    \node[font=\scriptsize\itshape, text=Slate] at (5.5,-1.8) {Ch.~10--11, Appendices};

\end{tikzpicture}
\captionof{figure}{Research question hierarchy. The main question integrates four sub-questions spanning theory (RQ1), method (RQ2), empirics (RQ3), and implications (RQ4). Dashed lines indicate bidirectional interdependencies: theoretical frameworks inform methodological design, which enables pattern discovery, which generates implications. Chapter mappings show where each question is primarily addressed.}
...
\begin{mainrq}
How can role constellations be studied empirically at scale---moving from theory-driven typologies to data-driven, theory-guided mapping---and what are the implications of multimodal cartography for navigating sustainability transitions?
\end{mainrq}

This question combines theoretical, methodological, empirical, and practical ambitions.\sidenote{\textbf{Four dimensions}---Theory (conceptual framework), method (multimodal cartography), empirics (EIT patterns), and implications (for governance).} It asks how to \emph{think about} role constellations, how to \emph{map} them systematically, what patterns \emph{emerge}, and what this \emph{implies} for navigating transitions. Four sub-questions structure the inquiry:
...
\begin{subrq}
\textbf{Empirics}\\
What role patterns emerge when multimodal cartography is applied to the EIT ecosystem?
\end{subrq}

\begin{subrq}
\textbf{Implications}\\
What do these patterns imply for navigating sustainability transitions?
\end{subrq}
...
\textbf{Empirical contributions} reveal role constellation patterns across eight intermediation domains.\sidenote{800+ orgs, 8 communities, 14 years, 6 modalities.} Key findings include the \emph{connector gap} (aspiration-reality divergence), \emph{knowledge triangle imbalance} (business dominance), and \emph{five recurring sociotypes} that emerge across domains.

\textbf{Infrastructural contributions} create the foundations for cumulative knowledge development.\sidenote{Open-source \RoleBox{}; CARTALOG; \RoleSim{}; replication packages.} \RoleBox{} provides an open-source platform with documented pipelines for six data modalities. \textbf{CARTALOG}, the master organizational catalog, links 8,600+ organizations across all data modalities---enabling researchers to trace how the same organization is portrayed across websites, social media, news, and visual materials. This infrastructure supports both computational queries and theory-driven exploration, addressing calls for ``data-driven transitions research'' with reusable artifacts.
