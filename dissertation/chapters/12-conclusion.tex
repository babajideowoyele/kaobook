...
This dissertation answered one question: \emph{How should data-driven, theory-guided workflows be designed to enable multimodal cartography of interstitial role constellations in sustainability transitions?} The answer is a workflow-centric IS bridging methodology \parencite{maass2018data} that occupies the space where Big Data meets domain theory. Three findings substantiate it: a \emph{connector gap} between claimed and enacted bridging roles; a \emph{Knowledge Triangle imbalance} (business 49\%, research 37\%, education 14\%) challenging the EIT mandate; and \emph{five website sociotypes} that single-modality analysis cannot reveal. \RoleBox{}---the demonstrative artifact---operationalizes this methodology across seven modalities, producing replicable design knowledge for transition governance and a template for cumulative, open computational social science.
\end{chapteroverview}
...
\begin{tikzpicture}[scale=1.0,
    ...
    \node[rqbox=PandaCharcoal, fill=PandaCharcoal!20, minimum width=10cm, minimum height=1.6cm] (main) at (0,4) {
        \textbf{Main RQ: How should data-driven, theory-guided workflows be designed}\\
        \textbf{to enable multimodal cartography of interstitial role constellations in ST?}
    };
    ...
    \node[rqbox=Azure] (rq1) at (-5.5,1.2) {\textbf{RQ1: Theory}\\[0.1em]\scriptsize Theory-guided,\\empirically open?};
    \node[rqbox=PandaBamboo] (rq2) at (-1.8,1.2) {\textbf{RQ2: Method}\\[0.1em]\scriptsize Cross-modal mapping\\of roles?};
    \node[rqbox=Marigold] (rq3) at (1.8,1.2) {\textbf{RQ3: Empirics}\\[0.1em]\scriptsize Role constellation\\patterns?};
    \node[rqbox=AccentPurple] (rq4) at (5.5,1.2) {\textbf{RQ4: Implications}\\[0.1em]\scriptsize DSR + open science\\barriers \& infrastructure?};
    ...
    \node[ansbox=Marigold] (a3) at (1.8,-0.8) {Interstitial pluralism\\KT imbalance\\5 sociotypes};
    \node[ansbox=AccentPurple] (a4) at (5.5,-0.8) {DSR workflows\\Open science\\Cumulative infrastructure};
...
\end{tikzpicture}
\captionof{figure}{Research question structure and answers. The main research question decomposes into four sub-questions, each addressed by specific dissertation chapters. RQ1 (theory) developed CAS reconceptualization with 15 propositions; RQ2 (method) built \RoleBox{} with six modality pipelines; RQ3 (empirics) revealed interstitial pluralism, knowledge triangle imbalance, and five recurring sociotypes across the EIT ecosystem; RQ4 (implications) delivered design science workflows that break down open science barriers while building up cumulative research infrastructure.}
...
\noindent\textbf{Answer.} Data-driven, theory-guided workflows should be designed around the \textbf{Maass bridging framework} (Curation $\to$ Annotation $\to$ Unsupervised $\to$ Output), grounded in \textbf{complex adaptive systems} theory that bridges transitions theory with computational methods \cite{holland1995hidden,kauffman1993origins}.\sidenote{\textbf{The CAS bridge}---Complexity science provides mechanisms that are both theoretically meaningful and computationally tractable---exactly the bridge transitions research needs \parencite{holland1995hidden,kauffman1993origins}.} The CAS framing offers mechanisms---emergence, self-organization, co-evolution, non-linearity, feedback loops---that explain why constellations exhibit the patterns they do while providing tractable formalizations for computational analysis. The \RoleBox{} infrastructure, with its six modality pipelines and integration protocols, provides the methodological artifacts that operationalize this framing.
...
\item \textbf{Knowledge triangle imbalance}: Business vocabulary (49\%) dominates over research (37\%) and education (14\%) across all domains, despite the EIT's founding mission of balanced integration.
...
\textbf{Interstitial pluralism} (Chapter~\ref{ch:interstitial}): EIT organizations strategically occupy bridging positions between established categories---research/business, local/global, public/private.\sidenote{\textbf{Interstitial actors}---Organizations that derive advantage from occupying positions \emph{between} rather than \emph{within} established categories.} This interstitial positioning allows them to translate between worlds, broker connections, and navigate institutional complexity. Rather than fitting neatly into existing typologies, these actors maintain deliberate ambiguity that enables adaptive responses to multiple stakeholders.
...
\textbf{Build-up bias} (Chapter~\ref{ch:power}): Analysis of verb connotations in news coverage reveals a striking absence of breakdown and exnovation roles.\sidenote{\textbf{X-curve framework}---Transitions require both acceleration of alternatives (build-up) and managed decline of incumbents (breakdown) \parencite{hebinck2023untapped}.} Across 1,273 articles analyzing EIT discourse, organizations are consistently portrayed through verbs of creation, growth, and acceleration---but never through verbs of phase-out, decline, or managed exit. This suggests innovation intermediaries are structurally oriented exclusively toward build-up dynamics, leaving the breakdown side of the X-curve unaddressed.
...
%-------------------------------------------------------------------------------
\subsection{Sub-Question 4: Infrastructure and Accumulation}
%-------------------------------------------------------------------------------
...
\noindent\textbf{Answer.} The dissertation operationalizes open science principles through multiple infrastructural contributions:\sidenote{\textbf{Open science}---Transparency and reproducibility are not afterthoughts but core design principles. Others should be able to validate, critique, and extend this work.}

\textbf{Open-source repositories}:
\begin{itemize}[leftmargin=*, itemsep=0.2em]
\item \textbf{RoleBox} (umbrella): \texttt{github.com/babajideowoyele/rolebox}
\item \textbf{RoleSim} (simulation): \texttt{github.com/babajideowoyele/rolesim}
\item \textbf{RoleXray} (NLP/extraction): \texttt{github.com/babajideowoyele/rolexray}
\item Data repositories for each modality
\end{itemize}

\textbf{Documentation standards}:
\begin{itemize}[leftmargin=*, itemsep=0.2em]
\item Pipeline specifications with input/output schemas
\item Parameter documentation and default justifications
\item Known limitations and edge cases
\item Example workflows with sample data
\end{itemize}

\textbf{Replication support}:
\begin{itemize}[leftmargin=*, itemsep=0.2em]
\item Version-pinned dependencies
\item Containerized execution environments
\item Test suites for validation
\item Step-by-step tutorials
\end{itemize}

The answer to Sub-Question 4 is that accumulation requires treating infrastructure as a first-class research output, not an afterthought. This means investing in documentation, testing, and accessibility alongside analytical capabilities.
...
\begin{tikzpicture}[scale=1.0,
    ...
    \node[detailbox=Azure, below=0.2cm of theory] {
        ...
        \textbullet\ IS/sociomaterial grounding
    };
...
\end{tikzpicture}
\captionof{figure}{Four contribution dimensions mutually reinforcing multimodal cartography. Theory (CAS reconceptualization, 15 propositions, IS/sociomaterial grounding) provides the conceptual foundation; methodology (\RoleBox{}, 6 pipelines) provides the tools; empirical work (8 KICs, connector gap, 5 sociotypes) demonstrates value; infrastructure (open source, documentation) enables accumulation. Information Systems serves as the metatheoretical paradigm unifying all four dimensions: DSR supplies the methodology; sociomateriality supplies the ontology.}
...
The third theoretical contribution is metatheoretical: \textbf{Information Systems as the paradigmatic home for multimodal cartography in sustainability transitions}.\sidenote{\textbf{IS as metatheory}---DSR supplies the methodology; sociomateriality supplies the ontology; sustainability transitions supply the domain. IS unifies all three.} By adopting a sociomaterial stance \cite{Orlikowski2007,orlikowski2008sociomateriality,leonardi2012materiality}, we demonstrate that cross-modal divergence---the connector gap, the knowledge triangle imbalance, the visual-verbal disconnect---is not a measurement limitation but an \emph{ontological consequence} of platform materiality. Different digital infrastructures constitute different role realities because their affordances make different aspects of role performance visible and enactable. This reframing transforms the dissertation's empirical findings from ``interesting patterns'' into evidence for a theoretical claim about the sociomaterial constitution of organisational roles.

This IS grounding also transforms how we understand the dissertation's core DSR artifacts. The computational workflows are not neutral pipelines but sociomaterial assemblages that co-produce role knowledge. The datasets are not raw material but digitally mediated traces shaped by platform architectures. The Meta-Design Requirements and design principles articulated in Chapter~\ref{ch:synthesis} are IS design knowledge in the full sense: transferable insights about building sociomaterial artifacts for complex social phenomena \cite{cecezkecmanovic2014sociomateriality}. And the entanglement runs deeper still: as \textcite{owoyele2025datacentering} argue, sustainability transitions and digital transformation are themselves constitutively entangled---the data centres and cloud platforms through which we study transitions are simultaneously infrastructure \emph{for} transitions and infrastructure \emph{requiring} transition. IS provides the metatheoretical vocabulary to theorise this reflexive condition.
...
The strongest evidence for the methodology's generality comes from Chapter~\ref{ch:video}, where the workflow-centric DSR approach transfers to a seventh modality---audio-visual data---and a different domain entirely: behavioral science.\sidenote{\textbf{Transferability}---The design cycle from Masked-Piper \parencite{owoyele2022maskedpiper} through MaskAnyone \parencite{owoyele2024maskanyone} to the AMP Pipeline \parencite{owoyele2025amppipeline} demonstrates that the Maass bridging framework (Curation $\rightarrow$ Annotation $\rightarrow$ Unsupervised $\rightarrow$ Output) is not specific to text or the EIT ecosystem.} The same Maass bridging framework that structures website analysis and news media processing structures video pose estimation and behavioral time series extraction. The same design cycle logic---build, evaluate, iterate---that produced \RoleBox{} produced three progressively more capable video workflow artifacts. And the same sociomaterial insight applies: computational infrastructure does not neutrally record behavioral phenomena but co-constitutes what becomes observable, analyzable, and shareable. This is not a post-hoc extension but evidence that the methodology produces transferable design knowledge---the hallmark of DSR contributions in Information Systems.
...
Second, the knowledge triangle tips toward business. EIT was founded to integrate research, education, and business equally. Vocabulary analysis shows business language (43\%) consistently outweighs research (32\%) and education (25\%). The triangle has a heavy side.
...
The combination reveals things single methods miss.\sidenote{\textbf{Synergistic insight}---No single modality would have revealed the connector gap.} Website analysis alone would have shown self-descriptions. Network analysis alone would have shown structural positions. Only the combination revealed the gap between them. The multimodal approach justified itself.

Opening the infrastructure was the right call \cite{salganik2018bit}. Others can now run our code, break it, fix it, extend it. Science accumulates through shared tools. Hoarding methods slows everyone down.
...
\item \textbf{Knowledge triangle imbalance}: Business (43\%) dominates over research (32\%) and education (25\%)
