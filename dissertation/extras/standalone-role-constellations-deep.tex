% Standalone document for Role Constellations Deep chapter
% Can be compiled independently with: lualatex standalone-role-constellations-deep.tex
...
\definecolor{Marigold}{RGB}{234,162,33}
\definecolor{PandaBamboo}{RGB}{76,153,0}
\definecolor{AccentPurple}{RGB}{128,0,128}
\definecolor{CoverGold}{RGB}{212,175,55}
...
% Document info
\title{\textbf{Role Constellations in Sustainability Transitions}\\[0.5cm]
\large Draft Chapter for Review}
\author{RoleBox Dissertation}
\date{\today}
...
\begin{enumerate}[noitemsep,topsep=0pt]
    ...
    \item \textbf{Incumbent peripheral actors}: Investigate how actors from adjacent regimes (banks, universities, regulators) shape transitions.
...
% Legend
\node[anchor=north west, font=\tiny, text width=14cm] at (-0.5,-3.2) {
...
The EIT comprises eight Knowledge and Innovation Communities, each an intermediary anchoring its own constellation:
\begin{itemize}[noitemsep]
\item Climate-KIC (2010), Digital (2010), InnoEnergy (2010)
\item Health (2015), Raw Materials (2016), Food (2017)
\item Manufacturing (2019), Urban Mobility (2019)
\end{itemize}

Rather than assuming we know what patterns exist, we pose open questions: \textit{What do we see when we look across these intermediaries? What modalities help us perceive imbrications---the overlapping, interlocking patterns across domains?}
...
\end{tikzpicture}
\caption{Eight EIT KICs as probes into different sustainability domains}
...
\end{tikzpicture}
\caption{Partner overlap matrix across eight KICs (illustrative)}
...
\begin{tabular}{@{}p{2.5cm}p{4.2cm}p{5cm}@{}}
\toprule
...
\node[anchor=north west, font=\scriptsize, text width=4cm, gray] at (-6,-3) {
    \textbf{Observation:} Brokers (purple) bridge otherwise disconnected clusters.
};
\end{tikzpicture}
\caption{Ground-level actor connections in a Deep Demonstration site}
